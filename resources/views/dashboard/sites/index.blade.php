...
@section('breadcrumb')
    ...
    <span class="font-medium text-inkSoft">Site-uri</span>
@endsection
...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-ink">Site-urile tale</h1>
            <p class="mt-1 text-sm text-muted">Gestionează site-urile pe care vrei să folosești agentul AI.</p>
        </div>
        @if($canAddSite ?? true)
                    ...
                    <div class="p-5">
                        ...
                        <div class="flex items-start justify-between mb-3">
                            <div class="min-w-0 flex-1">
                                <a href="{{ route('dashboard.sites.show', $site) }}" class="text-lg font-semibold text-ink hover:text-coralh transition-colors truncate block">
                                    {{ $site->domain }}
                                </a>
                                @if($site->name)
                                    <p class="text-sm text-muted truncate mt-0.5">{{ $site->name }}</p>
                                @endif
                            </div>
                            @php
                                $statusConfig = [
                                    ...
                                    'suspended' => ['label' => 'Suspendat', 'dot' => 'bg-red-500', 'text' => 'text-coral'],
                                ];
                        ...
                        <div class="flex items-center gap-2 pt-4 border-t border-line">
                            <a href="{{ route('dashboard.sites.show', $site) }}" title="Vezi detalii"
                               class="inline-flex items-center justify-center gap-1.5 rounded-lg border border-line bg-white px-3 py-2 text-sm font-medium text-muted hover:bg-cream hover:text-ink transition-colors">
    ...
    @else
        ...
        <div class="flex flex-col items-center justify-center py-16 px-4">
            ...
            <h3 class="text-lg font-semibold text-ink mb-1">Nu ai niciun site încă</h3>
            <p class="text-sm text-muted mb-6 text-center max-w-sm">Adaugă primul tău site pentru a putea folosi agentul AI.</p>
            @if($canAddSite ?? true)
                <a href="{{ route('dashboard.sites.create') }}"

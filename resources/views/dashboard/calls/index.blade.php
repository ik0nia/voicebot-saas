...
@section('breadcrumb')
    ...
    <span class="font-medium text-inkSoft">Apeluri</span>
@endsection
...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-ink">Apeluri</h1>
            <p class="mt-1 text-sm text-muted">{{ $calls->total() }} apeluri in total</p>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('dashboard.calls.index') }}" class="mb-6">
        <div class="rounded-xl border border-line bg-white p-4 shadow-sm">
            <div class="flex flex-wrap gap-3">
                ...
                {{-- Bot filter --}}
                <select name="bot"
                        class="rounded-lg border border-line bg-white px-4 py-2.5 text-sm text-inkSoft focus:border-coral focus:ring-2 focus:ring-coral/20 outline-none transition min-w-[160px]">
                ...
                {{-- Status filter --}}
                <select name="status"
                        class="rounded-lg border border-line bg-white px-4 py-2.5 text-sm text-inkSoft focus:border-coral focus:ring-2 focus:ring-coral/20 outline-none transition min-w-[150px]">
                ...
                <select name="direction"
                        class="rounded-lg border border-line bg-white px-4 py-2.5 text-sm text-inkSoft focus:border-coral focus:ring-2 focus:ring-coral/20 outline-none transition min-w-[140px]">
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr class="border-b border-line bg-cream/50">
                            <th class="px-5 py-3 font-medium text-muted">#</th>
                            <th class="px-5 py-3 font-medium text-muted">Agent AI</th>
                            <th class="px-5 py-3 font-medium text-muted">Apelant</th>
                            <th class="px-5 py-3 font-medium text-muted">Direcție</th>
                            <th class="px-5 py-3 font-medium text-muted">Status</th>
                            <th class="px-5 py-3 font-medium text-muted">Durată</th>
                            <th class="px-5 py-3 font-medium text-muted">Sentiment</th>
                            @if(auth()->user()->isSuperAdmin())
                            <th class="px-5 py-3 font-medium text-muted">Cost</th>
                            @endif
                            <th class="px-5 py-3 font-medium text-muted">Data</th>
                    ...
                    <tbody class="divide-y divide-line">
                        @foreach($calls as $call)
                            <tr class="hover:bg-cream/50 transition-colors">
                                ...
                                {{-- Caller number --}}
                                <td class="whitespace-nowrap px-5 py-3 text-muted font-mono">
                                    {{ $call->caller_number ?? '—' }}
                                </td>
                                ...
                                <td class="whitespace-nowrap px-5 py-3">
                                    @if($call->direction === 'inbound')
                                        <span class="inline-flex items-center gap-1 rounded-full bg-coralsoft px-2.5 py-0.5 text-xs font-medium text-coralh">
                                ...
                                <td class="whitespace-nowrap px-5 py-3">
                                    @php
                                        $statusConfig = [
                                            'initiated'   => ['label' => 'Inițiat',       'bg' => 'bg-cream',   'text' => 'text-muted'],
                                            ...
                                            'no_answer'   => ['label' => 'Fără răspuns',  'bg' => 'bg-cream',   'text' => 'text-muted'],
                                            'canceled'    => ['label' => 'Anulat',        'bg' => 'bg-cream',   'text' => 'text-muted'],
                                        ];
                                        $cfg = $statusConfig[$call->status] ?? ['label' => $call->status, 'bg' => 'bg-cream', 'text' => 'text-muted'];
                                    @endphp
                                ...
                                {{-- Date --}}
                                <td class="whitespace-nowrap px-5 py-3 text-muted" title="{{ $call->created_at->format('d.m.Y H:i:s') }}">
    ...
    @else
        ...
        <div class="flex flex-col items-center justify-center py-16 px-4">
            ...
            <h3 class="text-lg font-semibold text-ink mb-1">Niciun apel găsit</h3>

<div class="p-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <h3 class="text-lg font-semibold text-ink">Toate documentele</h3>
            <p class="text-sm text-muted">Lista completă a bazei de cunoștințe cu toate sursele.</p>
        </div>
        {{-- Source filter --}}
        <div class="flex flex-wrap items-center gap-2">
            <button onclick="filterDocs('all', this)" class="doc-filter-btn px-3 py-1.5 text-xs font-medium rounded-full bg-coral text-white transition-colors">Toate</button>
            <button onclick="filterDocs('manual', this)" class="doc-filter-btn px-3 py-1.5 text-xs font-medium rounded-full bg-cream text-muted hover:bg-sand transition-colors">Manual</button>
            <button onclick="filterDocs('agent', this)" class="doc-filter-btn px-3 py-1.5 text-xs font-medium rounded-full bg-cream text-muted hover:bg-sand transition-colors">Agent AI</button>
            <button onclick="filterDocs('scan', this)" class="doc-filter-btn px-3 py-1.5 text-xs font-medium rounded-full bg-cream text-muted hover:bg-sand transition-colors">Scanner</button>
            <button onclick="filterDocs('connector', this)" class="doc-filter-btn px-3 py-1.5 text-xs font-medium rounded-full bg-cream text-muted hover:bg-sand transition-colors">Conector</button>
        </div>
    </div>

    @if($documents->isEmpty())
        <div class="text-center py-12">
            <svg class="w-16 h-16 text-line mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
            <h3 class="text-lg font-semibold text-ink mb-2">Baza de cunoștințe este goală</h3>
            <p class="text-sm text-muted mb-6">Folosește agenții AI, upload sau scannerul pentru a adăuga conținut.</p>
    ...
    @else
        <div class="space-y-2" id="documents-list">
            @foreach($documents as $doc)
                <div class="doc-item flex items-center justify-between bg-white rounded-lg border border-line px-5 py-3.5 hover:shadow-sm transition-shadow" data-source="{{ $doc->source_type ?? 'manual' }}">
                    <div class="flex items-center gap-4 min-w-0">
                        ...
                        <div class="min-w-0">
                            <h4 class="text-sm font-semibold text-ink truncate">{{ $doc->title }}</h4>
                            <div class="flex flex-wrap items-center gap-2 mt-1">
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-cream text-muted">{{ strtoupper($doc->type) }}</span>
                                ...
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium
                                    ...
                                    @elseif($doc->status === 'pending') bg-cream text-muted
                                ...
                                @if($doc->status === 'failed' && $doc->error_message)
                                    <span class="text-xs text-coral truncate max-w-[200px]" title="{{ $doc->error_message }}">{{ Str::limit($doc->error_message, 60) }}</span>
                                @endif
                    ...
                    <form action="/dashboard/boti/{{ $bot->id }}/knowledge/{{ urlencode($doc->title) }}" method="POST" onsubmit="return confirm('Sigur dorești să ștergi acest document?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="shrink-0 p-2 rounded-lg text-muted hover:text-coral hover:bg-coralsoft transition-colors">
...
<script>
    ...
    (function() {
        var hasPending = document.querySelectorAll('.doc-item [class*="bg-yellow-100"], .doc-item [class*="bg-cream"]').length > 0;
        if (hasPending) {

...
<div id="agent-modal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute inset-y-0 right-0 w-full max-w-2xl bg-white shadow-xl flex flex-col">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-line">
            <div>
                <h2 id="agent-modal-title" class="text-lg font-semibold text-ink"></h2>
                <p id="agent-modal-role" class="text-sm text-muted mt-0.5"></p>
            </div>
            <button onclick="closeAgentModal()" class="p-2 rounded-lg text-muted hover:text-muted hover:bg-cream transition-colors">
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div id="agent-input-section">
                <label for="agent-user-input" class="block text-sm font-medium text-inkSoft mb-1.5">Descrie business-ul, produsul sau contextul</label>
                ...
                <div class="mt-4">
                    <button onclick="toggleCustomPrompt()" class="text-xs text-muted hover:text-coralh transition-colors flex items-center gap-1">
                    ...
                    <div id="custom-prompt-area" class="hidden mt-3">
                        <textarea id="agent-custom-prompt" rows="4"
                            class="w-full rounded-lg border border-line px-3.5 py-2.5 text-xs font-mono text-inkSoft placeholder-slate-400 focus:border-coral focus:ring-2 focus:ring-coral/20 outline-none transition resize-y"
                            placeholder="Promptul custom al agentului..."></textarea>
                    </div>
                </div>

                <div class="mt-4 flex justify-end">
                    <button onclick="runAgent()" id="btn-run-agent" class="inline-flex items-center gap-2 px-5 py-2.5 bg-coral text-white text-sm font-semibold rounded-lg hover:bg-coralh transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed">
            ...
            <div id="agent-loading-section" class="hidden text-center py-12">
                <div class="inline-flex flex-col items-center gap-3">
                    <svg class="w-5 h-5 animate-spin text-coralh" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"/></svg>
                    <span class="text-sm text-muted">Agentul generează conținut...</span>
                    <span id="agent-poll-counter" class="text-xs text-muted"></span>
                    <button onclick="cancelAgentRun()" class="mt-3 inline-flex items-center gap-1.5 px-4 py-2 bg-white text-muted text-sm font-medium rounded-lg border border-line hover:bg-cream transition-colors">
            ...
            <div id="agent-error-section" class="hidden text-center py-12">
                ...
                <h3 class="text-lg font-semibold text-ink mb-2">Generarea a expirat</h3>
                <p class="text-sm text-muted mb-4">Agentul nu a reușit să termine în timpul alocat. Te rog încearcă din nou.</p>
                <button onclick="resetAgentModal()" class="inline-flex items-center gap-2 px-4 py-2.5 bg-coral text-white text-sm font-semibold rounded-lg hover:bg-coralh transition-colors">
            ...
            <div id="agent-result-section" class="hidden">
                <div class="flex items-center justify-between mb-3">
                    <h3 class="text-sm font-semibold text-ink">Conținut generat</h3>
                    <span id="agent-tokens-badge" class="text-xs text-muted"></span>
                </div>
                <div id="agent-result-content" class="prose prose-sm max-w-none bg-cream rounded-lg p-4 border border-line text-sm text-inkSoft whitespace-pre-wrap max-h-96 overflow-y-auto"></div>
                ...
                <div class="mt-4 flex items-center gap-3">
                    ...
                    <button onclick="resetAgentModal()" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white text-inkSoft text-sm font-medium rounded-lg border border-line hover:bg-cream transition-colors">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
                        Regenerează
                    </button>
                </div>
            ...
            <div id="agent-saved-section" class="hidden text-center py-12">
                ...
                <h3 class="text-lg font-semibold text-ink mb-2">Salvat cu succes!</h3>
                <p class="text-sm text-muted">Conținutul a fost adăugat în baza de cunoștințe și se procesează pentru vectorizare.</p>
                <div class="mt-6 flex items-center justify-center gap-3">
                    <button onclick="resetAgentModal()" class="inline-flex items-center gap-2 px-4 py-2.5 bg-coral text-white text-sm font-semibold rounded-lg hover:bg-coralh transition-colors">
                        Generează alt conținut
                    </button>
                    <button onclick="goToDocuments()" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white text-inkSoft text-sm font-medium rounded-lg border border-line hover:bg-cream transition-colors">

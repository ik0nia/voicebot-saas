        ...
        <div class="bg-white rounded-xl border border-line shadow-sm">
            <div class="px-5 py-4 border-b border-line flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-base font-semibold text-ink">Integrare WordPress</h2>
                </div>
                @if($wcConnector)
            ...
            <div class="p-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-inkSoft mb-1.5">Cheie API</label>
                                ...
                                <div class="flex items-center gap-2">
                                    <div class="flex-1 relative">
                                        <input type="text" readonly value="{{ $latestToken->name }} ({{ Str::mask($latestToken->token ?? '****', '*', 4) }})" id="api-key-display"
                                            class="w-full rounded-lg border border-line bg-cream px-3 py-2 text-sm font-mono text-muted pr-10">
                                        <button onclick="navigator.clipboard.writeText(document.getElementById('api-key-display').value).then(()=>{this.textContent='OK';setTimeout(()=>{this.textContent='Copiaza'},1500)})"
                                            class="absolute right-2 top-1/2 -translate-y-1/2 text-xs font-medium text-coralh hover:text-coralh transition-colors">
                                            Copiaza
                                        </button>
                                    </div>
                                </div>
                                <p class="text-xs text-muted mt-1">Foloseste aceasta cheie in plugin-ul WordPress.</p>
                            @else
                                <p class="text-sm text-muted mb-2">Nu ai nicio cheie API generata.</p>
                                <a href="{{ route('dashboard.settings.index', ['tab' => 'api']) }}"
                                   class="inline-flex items-center gap-1.5 text-sm font-medium text-coralh hover:text-coralh transition-colors">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-inkSoft mb-1.5">Plugin WooCommerce</label>
                    ...
                    <div>
                        ...
                        @endphp
                        <label class="block text-sm font-medium text-inkSoft mb-1.5">Cod Embed Agent AI</label>
                            ...
                            <div class="relative group">
                                <div class="flex items-start gap-2">
                                    ...
                                    <button
                                        ...
                                        data-code="{{ $embedCode }}"
                                        class="shrink-0 inline-flex items-center gap-1.5 rounded-lg bg-coral hover:bg-coralh px-3 py-2 text-xs font-medium text-white transition-colors">
                                        Copiaza
                                    </button>
                                </div>
                            </div>
                            <p class="text-xs text-muted mt-1.5">Domeniu autorizat: <span class="font-medium">{{ $bot->site->domain }}</span></p>
                        @elseif($webChatbotChannel && !$siteVerified)
                            <div class="rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-700">
                                ...
                                @if($bot->site)
                                    <a href="{{ route('dashboard.sites.show', $bot->site) }}" class="text-xs font-medium text-coralh hover:text-coralh mt-1 inline-block">Verifica site-ul &rarr;</a>
                                @endif
                            </div>
                        @else
                            <div class="rounded-lg bg-cream border border-line px-4 py-3 text-sm text-muted">
                                <p>Activeaza un canal Web Agent AI si verifica site-ul.</p>
                                <a href="{{ route('dashboard.bots.channels.index', $bot) }}" class="text-xs font-medium text-coralh hover:text-coralh mt-1 inline-block">Gestioneaza canale &rarr;</a>
                            </div>
                        @endif
                    </div>
                </div>

...
<div x-data="personalitySettings()" class="space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <template x-for="t in tones">
                    <button type="button" @click="form.tone = t.v; dirty = true"
                        :class="form.tone === t.v ? 'border-red-400 bg-coralsoft/60 shadow-md ring-1 ring-red-200 scale-[1.02]' : 'border-line hover:border-line hover:shadow-sm'"
                        ...
                        <p class="text-sm font-bold text-ink" x-text="t.l"></p>
                        <p class="text-[11px] text-muted leading-tight mt-1" x-text="t.d"></p>
                    </button>
                </template>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-line shadow-sm overflow-hidden">
        <div class="border-l-4 border-blue-500 p-5">
            <label class="block text-sm font-semibold text-ink mb-1">Lungimea raspunsurilor</label>
            <p class="text-xs text-muted mb-5">Cat de detaliate sunt raspunsurile?</p>
            <div class="relative px-4">
                <div class="h-4 rounded-full bg-sand relative overflow-hidden shadow-inner">
                ...
                <div class="flex justify-between mt-3">
                    <template x-for="(l, i) in verbLabels">
                        <button type="button" @click="verbIdx = i; form.verbosity = ['concise','detailed','verbose'][i]; dirty = true" class="flex flex-col items-center gap-1.5 transition-all" :class="verbIdx === i ? 'opacity-100' : 'opacity-40 hover:opacity-70'">
                            <div class="w-5 h-5 rounded-full border-2 transition-all shadow-sm" :style="verbIdx >= i ? 'background:' + verbLabels[i][2] + ';border-color:' + verbLabels[i][2] : ''" :class="verbIdx < i ? 'border-line bg-white' : ''"></div>
                            <span class="text-[10px] font-medium text-muted" x-text="['Scurt', 'Echilibrat', 'Detaliat'][i]"></span>
                        </button>
                    </template>
                </div>
            </div>
            <div class="mt-4 px-4 py-3 rounded-lg transition-all duration-300 border" :style="'background:' + verbLabels[verbIdx][2] + '15; border-color:' + verbLabels[verbIdx][2] + '30'">
                ...
                <p class="text-xs text-muted mt-0.5" x-text="verbLabels[verbIdx][1]"></p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-line shadow-sm overflow-hidden">
        <div class="border-l-4 border-purple-500 px-5 py-5">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-semibold text-ink">Foloseste emoji-uri</p>
            ...
            <div class="mt-4 grid grid-cols-2 gap-3">
                <div class="rounded-lg px-4 py-3 transition-all duration-200"
                     :class="form.emoji_allowed ? 'bg-purple-50 border border-purple-200 shadow-sm' : 'bg-cream border border-line'">
                    <p class="text-[10px] font-semibold uppercase tracking-wider mb-1" :class="form.emoji_allowed ? 'text-purple-600' : 'text-muted'">Cu emoji</p>
                    <p class="text-sm text-inkSoft">Buna! 👋 Cu ce te pot ajuta?</p>
                </div>
                <div class="rounded-lg px-4 py-3 transition-all duration-200"
                     :class="!form.emoji_allowed ? 'bg-purple-50 border border-purple-200 shadow-sm' : 'bg-cream border border-line'">
                    <p class="text-[10px] font-semibold uppercase tracking-wider mb-1" :class="!form.emoji_allowed ? 'text-purple-600' : 'text-muted'">Fara emoji</p>
                    <p class="text-sm text-inkSoft">Buna! Cu ce te pot ajuta?</p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-xl border border-line shadow-sm overflow-hidden">
        <div class="border-l-4 border-amber-500 p-5">
            <label class="block text-sm font-semibold text-ink mb-1">Cat de insistent vinde agentul AI?</label>
            <p class="text-xs text-muted mb-5">Cat de des sugereaza produse sau actiuni de cumparare?</p>
            <div class="relative px-4">
                <div class="h-4 rounded-full bg-sand relative overflow-hidden shadow-inner">
                ...
                <div class="flex justify-between mt-3">
                    <template x-for="(l, i) in ctaLabels">
                        <button type="button" @click="ctaIdx = i; form.cta_aggressiveness = ['soft','moderate','aggressive'][i]; dirty = true" class="flex flex-col items-center gap-1.5 transition-all" :class="ctaIdx === i ? 'opacity-100' : 'opacity-40 hover:opacity-70'">
                            <div class="w-5 h-5 rounded-full border-2 transition-all shadow-sm" :style="ctaIdx >= i ? 'background:' + ctaLabels[i][2] + ';border-color:' + ctaLabels[i][2] : ''" :class="ctaIdx < i ? 'border-line bg-white' : ''"></div>
                            <span class="text-[10px] font-medium text-muted" x-text="['Subtil', 'Echilibrat', 'Proactiv'][i]"></span>
                        </button>
                    </template>
                </div>
            </div>
            <div class="mt-4 px-4 py-3 rounded-lg transition-all duration-300 border" :style="'background:' + ctaLabels[ctaIdx][2] + '15; border-color:' + ctaLabels[ctaIdx][2] + '30'">
                ...
                <p class="text-xs text-muted mt-0.5" x-text="ctaLabels[ctaIdx][1]"></p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-line shadow-sm overflow-hidden">
        <div class="border-l-4 border-emerald-500 p-5">
            <label class="block text-sm font-semibold text-ink mb-1">Capturare date de contact</label>
            <p class="text-xs text-muted mb-5">Cat de insistent cere agentul AI emailul sau telefonul?</p>
            <div class="relative px-4">
                <div class="h-4 rounded-full bg-sand relative overflow-hidden shadow-inner">
                ...
                <div class="flex justify-between mt-3">
                    <template x-for="(l, i) in leadLabels">
                        <button type="button" @click="leadIdx = i; form.lead_aggressiveness = ['soft','moderate','aggressive'][i]; dirty = true" class="flex flex-col items-center gap-1.5 transition-all" :class="leadIdx === i ? 'opacity-100' : 'opacity-40 hover:opacity-70'">
                            <div class="w-5 h-5 rounded-full border-2 transition-all shadow-sm" :style="leadIdx >= i ? 'background:' + leadLabels[i][2] + ';border-color:' + leadLabels[i][2] : ''" :class="leadIdx < i ? 'border-line bg-white' : ''"></div>
                            <span class="text-[10px] font-medium text-muted" x-text="['Discret', 'Moderat', 'Insistent'][i]"></span>
                        </button>
                    </template>
                </div>
            </div>
            <div class="mt-4 px-4 py-3 rounded-lg transition-all duration-300 border" :style="'background:' + leadLabels[leadIdx][2] + '15; border-color:' + leadLabels[leadIdx][2] + '30'">
                ...
                <p class="text-xs text-muted mt-0.5" x-text="leadLabels[leadIdx][1]"></p>
            </div>
        </div>
    </div>
    ...
    {{-- Custom Messages --}}
    <div class="bg-white rounded-xl border border-line shadow-sm overflow-hidden">
        <button type="button" @click="showCustom = !showCustom" class="w-full px-5 py-4 flex items-center justify-between hover:bg-cream transition-colors">
            <div class="flex items-center gap-2.5">
                <div class="w-8 h-8 rounded-lg bg-cream flex items-center justify-center">
                ...
                <span class="text-sm font-semibold text-inkSoft">Mesaje personalizate</span>
            </div>
            <svg class="w-4 h-4 text-muted transition-transform" :class="showCustom ? 'rotate-180' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
        </button>
        <div x-show="showCustom" x-transition class="px-5 pb-5 space-y-4">
            <div>
                <label class="block text-xs font-medium text-muted mb-1">Cand nu stie raspunsul</label>
                <input type="text" x-model="form.fallback_message" @input="dirty = true" placeholder="Ex: Nu am gasit informatia, dar te pot pune in legatura cu echipa noastra." class="w-full rounded-lg border border-line px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 placeholder:text-muted">
            </div>
            <div>
                <label class="block text-xs font-medium text-muted mb-1">Cand transfera la om</label>
                <input type="text" x-model="form.escalation_message" @input="dirty = true" placeholder="Ex: Te transfer catre un coleg care te poate ajuta mai bine." class="w-full rounded-lg border border-line px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 placeholder:text-muted">
            </div>
        </div>
    </div>
        ...
        <div class="flex items-center justify-between rounded-xl bg-gradient-to-r from-red-600 to-red-500 text-white px-5 py-3 shadow-lg shadow-red-500/20">
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="button" @click="save()" :disabled="saving"
                        class="inline-flex items-center gap-2 rounded-lg bg-white px-5 py-2 text-sm font-semibold text-coral hover:bg-coralsoft disabled:opacity-50 transition-colors shadow-sm">

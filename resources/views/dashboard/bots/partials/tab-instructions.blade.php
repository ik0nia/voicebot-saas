...
<div x-data="instructionsTab()" class="space-y-6">
    ...
    <div x-show="step === 'choose'" x-transition class="bg-white rounded-xl border border-line shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-line">
            <h2 class="text-base font-semibold text-ink">Alege domeniul afacerii</h2>
            <p class="text-xs text-muted mt-0.5">Selecteaza un sablon pregatit sau descrie-ti afacerea pentru instruciuni personalizate.</p>
        </div>
        <div class="p-5">
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2.5 mb-4">
                ...
                <button type="button" @click="selectTemplate('{{ $key }}')"
                    class="text-left rounded-xl border-2 border-line p-3 transition-all hover:border-blue-300 hover:shadow-sm hover:bg-blue-50/30"
                    ...
                    <p class="text-xs font-semibold text-ink leading-tight">{{ $tpl['label'] }}</p>
                    <p class="text-[10px] text-muted mt-0.5 leading-tight line-clamp-2">{{ $tpl['description'] }}</p>
                </button>
                @endforeach
                {{-- Custom --}}
                <button type="button" @click="step = 'custom'"
                    class="text-left rounded-xl border-2 border-dashed border-line p-3 transition-all hover:border-blue-400 hover:bg-blue-50/30">
                    <div class="text-xl mb-1.5">✨</div>
                    <p class="text-xs font-semibold text-ink leading-tight">Alt domeniu</p>
                    <p class="text-[10px] text-muted mt-0.5 leading-tight">Descrie afacerea si AI genereaza</p>
                </button>
            </div>
            ...
            <div class="pt-4 border-t border-line">
                <div class="flex items-center justify-between mb-2">
                    <span class="text-xs font-medium text-muted">Instructiunile actuale ale agentului AI:</span>
                ...
                <div class="bg-cream rounded-lg p-3 text-xs text-muted max-h-24 overflow-hidden relative">
    ...
    <div x-show="step === 'custom'" x-transition class="bg-white rounded-xl border border-line shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-line flex items-center justify-between">
            <div>
                <h2 class="text-base font-semibold text-ink">Descrie afacerea ta</h2>
                <p class="text-xs text-muted mt-0.5">AI-ul va genera instructiuni personalizate pentru agentul AI tau.</p>
            </div>
            <button type="button" @click="step = 'choose'" class="text-xs text-muted hover:text-inkSoft flex items-center gap-1">
        ...
        <div class="p-5 space-y-5">
            <div>
                <label class="block text-sm font-medium text-ink mb-1">Numele afacerii</label>
                <input type="text" x-model="businessName" placeholder="Ex: Floraria Magnolia" class="w-full rounded-lg border border-line px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-ink mb-1">Cu ce se ocupa afacerea?</label>
                <textarea x-model="businessDesc" rows="3" placeholder="Ex: Vindem buchete de flori proaspete, aranjamente florale pentru evenimente si livram in tot orasul. Avem si plante de apartament." class="w-full rounded-lg border border-line px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500"></textarea>
            </div>
            <div>
                <label class="block text-sm font-medium text-ink mb-2">Ce ton vrei sa aiba agentul AI?</label>
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <template x-for="t in toneOptions">
                        <label class="cursor-pointer">
                            <input type="radio" x-model="customTone" :value="t.v" class="peer sr-only">
                            <div class="rounded-lg border-2 border-line p-2.5 text-center transition-all peer-checked:border-blue-500 peer-checked:bg-blue-50 hover:border-line">
                                <p class="text-xs font-semibold text-ink" x-text="t.l"></p>
                                <p class="text-[10px] text-muted" x-text="t.d"></p>
                            </div>
                        </label>
                    </template>
                </div>
    ...
    <div x-show="step === 'edit'" x-transition class="space-y-4">
        ...
        <div class="bg-white rounded-xl border border-line shadow-sm p-5">
            <label class="block text-sm font-medium text-ink mb-1">Mesaj de intampinare</label>
            <p class="text-[11px] text-muted mb-2">Primul mesaj pe care il vede clientul cand deschide chat-ul.</p>
            <input type="text" x-model="greeting" class="w-full rounded-lg border border-line px-3 py-2.5 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="Ex: Buna! Cu ce te pot ajuta?">
        </div>
        ...
        <div class="bg-white rounded-xl border border-line shadow-sm p-5">
            <div class="flex items-center justify-between mb-2">
                <div>
                    <label class="block text-sm font-medium text-ink">Instrucțiuni pentru agent</label>
                    <p class="text-[11px] text-muted">Regulile pe care le urmeaza agentul AI in fiecare conversatie.</p>
                </div>
                <span class="text-[10px] text-muted" x-text="prompt.length + ' caractere'"></span>
            </div>
            <textarea x-model="prompt" rows="14" class="w-full rounded-lg border border-line px-3 py-2.5 text-sm font-mono leading-relaxed focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="Descrie cum se comporta agentul AI..."></textarea>
        </div>

        {{-- Save --}}
        <div class="flex items-center gap-3">
            <button type="button" @click="saveAll()" :disabled="saving"
                    class="inline-flex items-center gap-2 rounded-lg bg-coral px-6 py-2.5 text-sm font-medium text-white hover:bg-coralh disabled:opacity-50 transition-colors shadow-sm">

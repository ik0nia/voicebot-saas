    @php
        ...
        $kbScoreColor = $kbScore >= 80 ? 'text-green-600' : ($kbScore >= 40 ? 'text-amber-500' : 'text-coral');
        ...
        <div class="bg-white rounded-xl border border-line shadow-sm">
            <div class="px-5 py-4 border-b border-line flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="text-base font-semibold text-ink">Knowledge Base</h2>
                </div>
                <div class="flex items-center gap-2">
                    @if(!empty($wcConnector) && ($wcConnector->status ?? null) === 'connected')
                        <a href="{{ route('dashboard.bots.wcMeta.index', $bot) }}"
                           class="inline-flex items-center gap-1.5 rounded-lg border border-line bg-white px-3.5 py-2 text-sm font-medium text-inkSoft hover:bg-cream transition-colors"
                    ...
                    <a href="{{ route('dashboard.bots.knowledge.index', $bot) }}"
                       class="inline-flex items-center gap-1.5 rounded-lg bg-coral px-4 py-2 text-sm font-medium text-white hover:bg-coralh transition-colors">
                    ...
                    <div class="flex flex-col items-center">
                        <div class="relative w-24 h-24 mb-3">
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center">
                                ...
                                <span class="text-[10px] text-muted uppercase tracking-wide">Scor</span>
                            </div>
                        </div>
                        <div class="text-center">
                            <p class="text-lg font-bold text-ink">{{ $kbStats['total_documents'] }}</p>
                            <p class="text-xs text-muted">documente ({{ $kbStats['total_chunks'] }} chunks)</p>
                        </div>
                    </div>

                    {{-- Recent documents --}}
                    <div class="md:col-span-2">
                        <h3 class="text-sm font-medium text-inkSoft mb-3">Documente recente</h3>
                        @if($recentKnowledge->count() > 0)
                            <ul class="space-y-2">
                                @foreach($recentKnowledge as $doc)
                                    <li class="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-cream transition-colors">
                                        <div class="flex items-center justify-center w-7 h-7 rounded bg-cream shrink-0">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <p class="text-sm font-medium text-ink truncate">{{ $doc->title }}</p>
                                            <p class="text-xs text-muted">{{ $doc->source_type ?? 'manual' }} &middot; {{ $doc->created_at->diffForHumans() }}</p>
                                        </div>
                                    </li>
                                @endforeach
                            </ul>
                        @else
                            <div class="rounded-lg bg-cream border border-line border-dashed px-4 py-6 text-center">
                                <p class="text-sm text-muted">Niciun document in Knowledge Base.</p>
                                <a href="{{ route('dashboard.bots.knowledge.index', $bot) }}" class="text-sm font-medium text-coralh hover:text-coralh mt-1 inline-block">Adauga documente &rarr;</a>
                            </div>
                        @endif
                    </div>
                </div>
    ...
    @php
        ...
        $hsIconBg = $hsScore >= 80 ? 'bg-green-50' : ($hsScore >= 60 ? 'bg-amber-50' : ($hsScore >= 40 ? 'bg-orange-50' : 'bg-coralsoft'));
        $hsIconText = $hsScore >= 80 ? 'text-green-600' : ($hsScore >= 60 ? 'text-amber-600' : ($hsScore >= 40 ? 'text-orange-600' : 'text-coral'));
        $hsBadgeBg = $hsScore >= 80 ? 'bg-green-100 text-green-700' : ($hsScore >= 60 ? 'bg-amber-100 text-amber-700' : ($hsScore >= 40 ? 'bg-orange-100 text-orange-700' : 'bg-coralsoft text-coralh'));
        $hsScoreText = $hsScore >= 80 ? 'text-green-600' : ($hsScore >= 60 ? 'text-amber-600' : ($hsScore >= 40 ? 'text-orange-600' : 'text-coral'));
    ...
    <div id="section-health" class="mb-6">
        <div class="bg-white rounded-xl border border-line shadow-sm">
            <div class="px-5 py-4 border-b border-line flex items-center gap-3">
                ...
                <h2 class="text-base font-semibold text-ink">Sănătatea Agentului AI</h2>
            ...
            <div class="p-5">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="flex flex-col items-center">
                        <div class="relative w-28 h-28 mb-3">
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center">
                                ...
                                <span class="text-[10px] text-muted uppercase tracking-wide">din 100</span>
                            </div>
                        </div>
                        <p class="text-sm font-medium text-inkSoft">Scor General</p>
                    </div>

                    {{-- Component Breakdown --}}
                    <div>
                        <h3 class="text-sm font-medium text-inkSoft mb-3">Componente</h3>
                        <div class="space-y-3">
                            @foreach($healthScore['components'] ?? [] as $key => $comp)
                                ...
                                <div>
                                    <div class="flex items-center justify-between text-xs mb-1">
                                        <span class="text-muted">{{ $comp['label'] }}</span>
                                        <span class="font-medium text-ink">{{ $comp['score'] }}/{{ $comp['max'] }}</span>
                                    </div>
                                    <div class="w-full h-1.5 bg-cream rounded-full">
                    ...
                    {{-- Suggestions --}}
                    <div>
                        <h3 class="text-sm font-medium text-inkSoft mb-3">Ce poți îmbunătăți</h3>
                        @forelse($healthScore['suggestions'] ?? [] as $suggestion)
                            <div class="flex items-start gap-2 py-2 {{ !$loop->last ? 'border-b border-line' : '' }}">
                                <span class="text-base mt-0.5 shrink-0">{{ $suggestion['icon'] ?? '💡' }}</span>
                                <div>
                                    <p class="text-xs text-inkSoft">{{ $suggestion['message'] }}</p>
                                </div>
                            </div>
                        @empty
    ...
    <div id="section-gaps" class="mb-6">
        <div class="bg-white rounded-xl border border-line shadow-sm">
            <div class="px-5 py-4 border-b border-line flex items-center gap-3">
                ...
                <h2 class="text-base font-semibold text-ink">Ce nu știe agentul AI</h2>
                <span class="ml-auto text-xs text-muted">Ultimele 7 zile &middot; {{ $knowledgeGaps['fail_rate'] }}% rată eșec</span>
            </div>
            <div class="p-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-5">
                        @foreach($knowledgeGaps['suggestions'] as $suggestion)
                            <div class="rounded-lg border border-line p-4 hover:border-coral/40 hover:bg-coralsoft/30 transition-colors">
                                ...
                                <p class="text-xs text-muted mb-2">{{ $suggestion['description'] }}</p>
                                <p class="text-xs text-muted">{{ $suggestion['occurrences'] }} întrebări fără răspuns</p>
                                @if(!empty($suggestion['sample_queries']))
                                    <div class="mt-2 pt-2 border-t border-line">
                                        <p class="text-[10px] uppercase tracking-wide text-muted mb-1">Exemple:</p>
                                        @foreach(array_slice($suggestion['sample_queries'], 0, 2) as $sq)
                                            <p class="text-xs text-muted italic">"{{ $sq }}"</p>
                                        @endforeach
                                    </div>
                                @endif
                            </div>
                ...
                @if(!empty($knowledgeGaps['gaps']))
                    <details class="group">
                        <summary class="text-sm font-medium text-inkSoft cursor-pointer hover:text-coralh transition-colors">
                        ...
                        <div class="mt-3 space-y-1">
                            @foreach(array_slice($knowledgeGaps['gaps'], 0, 10) as $gap)
                                <div class="flex items-center justify-between py-1.5 px-3 rounded hover:bg-cream text-xs">
                                    <span class="text-inkSoft">"{{ $gap['query'] }}"</span>
                                    <span class="text-muted shrink-0 ml-2">{{ $gap['occurrences'] }}x</span>
                                </div>
                            @endforeach
                        </div>
                    </details>

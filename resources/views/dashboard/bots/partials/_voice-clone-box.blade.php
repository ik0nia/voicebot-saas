...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
    ...
    @if($clonedVoice && $clonedVoice->isReady())
        <div class="flex items-center justify-between bg-slate-50 rounded-lg border border-slate-200 p-3">
            <div>
            ...
            <div class="flex items-center gap-2">
                ...
                @else
                    <button type="button" onclick="vcAction('{{ route('dashboard.bots.voiceClone.deactivate', $bot) }}', 'POST')" class="px-3 py-1.5 text-xs font-medium rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 transition">Revin-o la vocea presetată</button>
                @endif
                <button type="button" onclick="if(confirm('Sigur doriți să ștergeți această voce clonată?')) vcAction('{{ route('dashboard.bots.voiceClone.destroy', [$bot, $clonedVoice]) }}', 'DELETE')" class="p-1.5 text-red-400 hover:text-red-600" title="Șterge voce clonată">
    ...
    @elseif($clonedVoice && $clonedVoice->isPending())
        <div class="bg-slate-50 rounded-lg border border-yellow-200 p-3">
            <div class="flex items-center gap-2">
            ...
            <p class="text-xs text-slate-500 mt-1" id="clone-poll-msg">Se verifică automat... <span id="clone-poll-cd">5</span>s</p>
        </div>

    @elseif($clonedVoice && $clonedVoice->status === 'failed')
        <div class="bg-slate-50 rounded-lg border border-red-200 p-3 mb-3">
            <span class="text-sm text-red-700 font-medium">{{ $clonedVoice->name }} — eșuat</span>
            @if($clonedVoice->error_message)
                <p class="text-xs text-red-500 mt-0.5">{{ $clonedVoice->error_message }}</p>
            @endif
            <button type="button" onclick="vcAction('{{ route('dashboard.bots.voiceClone.destroy', [$bot, $clonedVoice]) }}', 'DELETE')" class="mt-2 text-xs text-red-600 underline hover:no-underline">Șterge și încearcă din nou</button>
        </div>
    @endif

    @if(!$clonedVoice || $clonedVoice->status === 'failed')
    <div id="record-ui" class="mt-3">
        <p class="text-xs text-slate-500 mb-3">Înregistrează-ți vocea citind textul de mai jos (minim 60 secunde):</p>

        <div class="bg-slate-50 border border-slate-200 rounded-lg p-3 mb-3 max-h-24 overflow-y-auto">
            <p class="text-xs text-slate-600 leading-relaxed">Bună ziua, mă numesc și sunt asistentul dumneavoastră virtual. Sunt aici pentru a vă ajuta cu orice întrebare sau solicitare. Compania noastră oferă servicii de înaltă calitate, personalizate pentru nevoile fiecărui client. Putem programa întâlniri, oferi informații despre produsele și serviciile noastre, sau vă putem pune în legătură cu un consultant specializat.</p>
        </div>

        <div class="flex items-center gap-3 mb-3">
            <input type="text" id="vc-name" placeholder="Numele vocii (ex: Vocea lui Andrei)" value="{{ old('name') }}"
                   class="flex-1 rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-red-700 focus:ring-2 focus:ring-red-700/20 outline-none">
        </div>

        <div class="flex items-center gap-3">
            <button type="button" id="vc-btn-record" onclick="vcStart()" class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold rounded-lg bg-red-600 text-white hover:bg-red-700">
        ...
        <button type="button" id="vc-btn-upload" onclick="vcUpload()" class="hidden mt-3 inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold rounded-lg bg-red-600 text-white hover:bg-red-700">

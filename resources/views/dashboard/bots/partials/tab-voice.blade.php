    ...
    <div class="mb-6">
        <div class="bg-white rounded-xl border border-line shadow-sm">
            <div class="px-5 py-4 border-b border-line flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-base font-semibold text-ink">Voce</h2>
                    <p class="text-xs text-muted">Setari pentru apelurile vocale si demo</p>
                </div>
            </div>
            <div class="p-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-muted mt-1">Folosita cand nu e activa o voce clonata</p>
                    </div>

                    {{-- Voice cloning --}}
                    <div>
                        <label class="block text-sm font-medium text-inkSoft mb-2">Voce clonata</label>
                        <div class="flex items-center justify-between p-3 rounded-lg bg-cream border border-line">
                            <div class="flex items-center gap-3">
                                ...
                                @elseif(isset($clonedVoice) && $clonedVoice && $clonedVoice->isReady())
                                    ...
                                    <div>
                                        <p class="text-sm font-medium text-inkSoft">{{ $clonedVoice->name }}</p>
                            ...
                            <div class="flex items-center gap-2">
                                @if(isset($clonedVoice) && $clonedVoice && $clonedVoice->isReady() && $bot->cloned_voice_id !== $clonedVoice->id)
                                    <button onclick="vcAction('{{ route('dashboard.bots.voiceClone.activate', [$bot, $clonedVoice]) }}', 'POST')"
                                        class="px-3 py-1.5 text-xs font-medium rounded-lg bg-coral text-white hover:bg-coralh transition-colors">Activeaza</button>
                                @elseif($bot->cloned_voice_id)
                                    <button onclick="vcAction('{{ route('dashboard.bots.voiceClone.deactivate', $bot) }}', 'POST')"
                                        class="px-3 py-1.5 text-xs font-medium rounded-lg border border-line text-muted hover:bg-white transition-colors">Dezactiveaza</button>
                                @endif
                                <a href="{{ route('dashboard.bots.voiceClone.create', $bot) }}"
                                   class="px-3 py-1.5 text-xs font-medium rounded-lg border border-line text-muted hover:bg-white transition-colors">
                ...
                <div class="mt-4 flex items-center gap-3">
                    <a href="{{ route('dashboard.bots.testVocal', $bot) }}"
                       class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg border border-coral/30 bg-coralsoft text-coralh hover:bg-coralsoft transition-colors">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        Testeaza vocal
                    </a>
                    <a href="{{ route('public.demo', $bot->slug) }}" target="_blank"
                       class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg border border-line text-muted hover:bg-cream transition-colors">

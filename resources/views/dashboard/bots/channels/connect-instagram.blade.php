...
@section('breadcrumb')
    ...
    <span class="text-slate-400">/</span>
    <span class="font-medium text-slate-700">Conectează Instagram</span>
@endsection
...
@section('content')
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-900">Conectează Instagram DM</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <form method="POST" action="{{ route('dashboard.bots.channels.instagram.store', $bot) }}" class="space-y-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                @csrf
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-500">Doar pentru identificare în dashboard. Dacă lași gol, folosim @username-ul.</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5" for="instagram_username">Username Instagram <span class="text-slate-400 font-normal">(opțional)</span></label>
                    <div class="relative">
                        ...
                        <input type="text" name="instagram_username" id="instagram_username" value="{{ old('instagram_username') }}" autocomplete="off"
                               class="w-full pl-7 rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-slate-500 focus:ring-1 focus:ring-slate-500" maxlength="32">
                    </div>
                    <p class="mt-1 text-xs text-slate-500">Doar pentru afișare. Sambla nu validează cu Instagram.</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5" for="instagram_id">Instagram Business Account ID</label>
                    ...
                    <p class="mt-1 text-xs text-slate-500">
                        Diferit de numărul de ID al contului tău Instagram. În Meta Graph API Explorer rulează <code class="text-[11px] bg-slate-100 px-1 rounded">/me/accounts?fields=instagram_business_account</code> cu Page Access Token.
                    </p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5" for="page_id">Page ID (pagina Facebook conectată)</label>
                    <input type="text" name="page_id" id="page_id" value="{{ old('page_id') }}" required pattern="[0-9]+" autocomplete="off"
                           class="w-full font-mono rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:border-slate-500 focus:ring-1 focus:ring-slate-500">
                    <p class="mt-1 text-xs text-slate-500">Instagram Business e linkat la o pagină Facebook. Page ID-ul ei e cel pe care îl trecem aici.</p>
                </div>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-500">
                        Token-ul system-user al paginii Facebook conectate, cu permisiunile <code class="text-[11px] bg-slate-100 px-1 rounded">instagram_basic</code> + <code class="text-[11px] bg-slate-100 px-1 rounded">instagram_manage_messages</code> + <code class="text-[11px] bg-slate-100 px-1 rounded">pages_messaging</code> + <code class="text-[11px] bg-slate-100 px-1 rounded">pages_show_list</code>.
                    </p>
                </div>
        ...
        <aside class="space-y-4">
            <div class="rounded-xl border border-slate-200 bg-slate-50 p-5">
                <h3 class="text-sm font-semibold text-slate-900 mb-3">Înainte să începi</h3>

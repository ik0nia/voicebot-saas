...
@section('breadcrumb')
    ...
    <span class="text-muted">/</span>
    <span class="font-medium text-inkSoft">Conectează WhatsApp</span>
@endsection
...
@section('content')
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-ink">Conectează WhatsApp Business</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <form method="POST" action="{{ route('dashboard.bots.channels.whatsapp.store', $bot) }}" class="space-y-5 rounded-xl border border-line bg-white p-6 shadow-sm">
                @csrf
                ...
                <div>
                    <label class="block text-sm font-medium text-inkSoft mb-1.5" for="waba_id">WABA ID</label>
                    ...
                    <p class="mt-1 text-xs text-muted">Business Manager → WhatsApp Accounts → ID-ul WhatsApp Business Account.</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-inkSoft mb-1.5" for="phone_number_id">Phone Number ID</label>
                    <input type="text" name="phone_number_id" id="phone_number_id" value="{{ old('phone_number_id') }}" required pattern="[0-9]+" autocomplete="off"
                           class="w-full font-mono rounded-lg border border-line px-3.5 py-2.5 text-sm focus:border-slate-500 focus:ring-1 focus:ring-slate-500">
                    <p class="mt-1 text-xs text-muted">WhatsApp Business → Phone Numbers → coloana „ID".</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-inkSoft mb-1.5" for="access_token">System User Access Token</label>
                    <textarea name="access_token" id="access_token" required rows="3" autocomplete="off"
                              class="w-full font-mono rounded-lg border border-line px-3.5 py-2.5 text-xs focus:border-slate-500 focus:ring-1 focus:ring-slate-500">{{ old('access_token') }}</textarea>
                    <p class="mt-1 text-xs text-muted">
                        Business Settings → Users → System Users → generează token cu permisiunile <code class="text-[11px] bg-cream px-1 rounded">whatsapp_business_messaging</code> + <code class="text-[11px] bg-cream px-1 rounded">whatsapp_business_management</code>. <span class="text-amber-600 font-medium">Token-ul rămâne activ permanent — păstrează-l confidențial.</span>
                    </p>
                </div>

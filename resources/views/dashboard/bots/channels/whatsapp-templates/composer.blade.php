...
@section('breadcrumb')
    ...
    <a href="{{ route('dashboard.bots.channels.whatsapp-templates.index', [$bot, $channel]) }}" class="text-slate-500 hover:text-slate-700 transition-colors">Template-uri</a>
    <span class="text-slate-400">/</span>
    <span class="font-medium text-slate-700">{{ $template ? $template->name : 'Nou' }}</span>
@endsection
...
@section('content')
    ...
    <form method="POST" action="{{ $action }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                <div class="rounded-xl border border-slate-200 bg-white p-5">
                    <h3 class="text-sm font-semibold text-slate-900 mb-4">Informații de bază</h3>

                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5" for="name">Nume template</label>
                            <input type="text" name="name" id="name" required pattern="[a-z0-9_]+" maxlength="64" {{ $isDraft ? '' : 'readonly' }}
                                   value="{{ old('name', $template->name ?? '') }}"
                                   class="w-full font-mono rounded-lg border border-slate-300 px-3 py-2 text-sm {{ $isDraft ? 'focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500' : 'bg-slate-50 cursor-not-allowed' }}">
                            <p class="mt-1 text-xs text-slate-500">Doar litere mici, cifre, underscore. Ex: <code>booking_confirmation</code></p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Categorie</label>
                            <div class="space-y-1.5">
                                @foreach(['MARKETING' => 'Marketing — promoții, anunțuri', 'UTILITY' => 'Utilitate — confirmări, reminders, statusuri', 'AUTHENTICATION' => 'Autentificare — coduri OTP'] as $key => $label)
                                    <label class="flex items-start gap-2 p-2 rounded border border-slate-200 cursor-pointer hover:bg-slate-50 {{ !$isDraft ? 'opacity-60 cursor-not-allowed' : '' }}">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5" for="language">Limbă</label>
                            <select name="language" id="language" required {{ !$isDraft ? 'disabled' : '' }}
                                    class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm {{ !$isDraft ? 'bg-slate-50' : '' }}">
            ...
            <div class="space-y-6">
                <div class="rounded-xl border border-slate-200 bg-white p-5">
                    <h3 class="text-sm font-semibold text-slate-900 mb-4">Conținut</h3>

                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5">Header (opțional)</label>
                            <select name="header_type" {{ !$isDraft ? 'disabled' : '' }}
                                    class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm mb-2 {{ !$isDraft ? 'bg-slate-50' : '' }}">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5" for="body_text">Body <span class="text-slate-400 text-xs">(folosește {{ '{{1}} {{2}} ...' }} pentru variabile)</span></label>
                            <textarea name="body_text" id="body_text" required rows="6" maxlength="1024" {{ !$isDraft ? 'readonly' : '' }}
                                      class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm {{ !$isDraft ? 'bg-slate-50' : '' }}">{{ old('body_text', $bodyComp['text'] ?? '') }}</textarea>
                            <p class="mt-1 text-xs text-slate-500">Max 1024 caractere. Ex: <code>Bună {{ '{{1}}' }}, programarea ta pentru {{ '{{2}}' }} e confirmată.</code></p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1.5" for="footer_text">Footer (opțional)</label>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-5">
                    <h3 class="text-sm font-semibold text-slate-900 mb-4">Butoane (max 10)</h3>
                    <p class="text-xs text-slate-500 mb-3">QUICK_REPLY trimite înapoi un postback. URL deschide un link. PHONE_NUMBER inițiază apel.</p>
                        ...
                        @for($i = 0; $i < 3; $i++)
                            ...
                            <div class="grid grid-cols-12 gap-2 items-center">
                                <select name="buttons[{{ $i }}][type]" {{ !$isDraft ? 'disabled' : '' }}
                                        class="col-span-3 rounded-lg border border-slate-300 px-2 py-1.5 text-xs {{ !$isDraft ? 'bg-slate-50' : '' }}">
                                ...
                                <input type="text" name="buttons[{{ $i }}][text]" maxlength="25" {{ !$isDraft ? 'readonly' : '' }}
                                       ...
                                       class="col-span-4 rounded-lg border border-slate-300 px-2 py-1.5 text-xs {{ !$isDraft ? 'bg-slate-50' : '' }}">
                                ...
                                <input type="text" name="buttons[{{ $i }}][phone_number]" {{ !$isDraft ? 'readonly' : '' }}
                                       ...
                                       placeholder="+40..."
                                       class="col-span-2 rounded-lg border border-slate-300 px-2 py-1.5 text-xs font-mono {{ !$isDraft ? 'bg-slate-50' : '' }}">
                            </div>
                        @endfor
                    </div>
                </div>
            ...
            <div class="space-y-6">
                @if($template && !empty($template->bodyVariables()))
                    <div class="rounded-xl border border-slate-200 bg-white p-5">
                        <h3 class="text-sm font-semibold text-slate-900 mb-4">Sample values pentru preview</h3>
                        @foreach($template->bodyVariables() as $varNum)
                            <div class="mb-3">
                                <label class="block text-xs font-medium text-slate-600 mb-1">Variabila {{ '{{' . $varNum . '}}' }}</label>
                ...
                <div class="rounded-xl border border-slate-200 bg-emerald-50/30 p-5">
                    ...
                    @endphp
                    <div class="rounded-lg bg-white border border-slate-200 p-4 max-w-xs ml-auto shadow-sm">
                        @if(!empty($headerComp['text']))
                            <p class="text-sm font-bold text-slate-900 mb-1">{{ $headerComp['text'] }}</p>
                        @endif
                        @if($previewBody)
                            <p class="text-sm text-slate-700 whitespace-pre-line">{{ $previewBody }}</p>
                        @else
                            <p class="text-sm text-slate-300 italic">— body apare aici —</p>
                        @endif
                        @if(!empty($footerComp['text']))
                            <p class="text-xs text-slate-500 mt-2">{{ $footerComp['text'] }}</p>
                        @endif
                        @foreach($buttonsComp['buttons'] ?? [] as $btn)
                            @if(!empty($btn['text']))
                                <button type="button" class="mt-2 w-full rounded border border-slate-300 px-3 py-1.5 text-xs text-emerald-700 font-medium">
                                    {{ $btn['text'] }}
                                </button>
                            @endif
                        @endforeach
        ...
        <div class="mt-8 flex items-center justify-between border-t border-slate-200 pt-6">
            <a href="{{ route('dashboard.bots.channels.whatsapp-templates.index', [$bot, $channel]) }}" class="text-sm text-slate-500 hover:text-slate-700">Anulează</a>

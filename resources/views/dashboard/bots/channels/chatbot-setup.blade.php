...
@section('breadcrumb')
    ...
    <span class="text-muted">/</span>
    <span class="font-medium text-inkSoft">Widget setup</span>
@endsection
...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-ink">Configurare widget</h1>
            <p class="mt-1 text-sm text-muted">Alege tema widget-ului și configurează mesajul de întâmpinare — previzualizarea se actualizează pe loc.</p>
        </div>
        <div class="flex items-center gap-2">
            <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium {{ $channel->is_active ? 'bg-green-50 text-green-700' : 'bg-cream text-muted' }}">
    ...
    <form method="POST" action="{{ route('dashboard.bots.channels.chatbot-setup.save', [$bot, $channel]) }}">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl border border-line shadow-sm">
                    <div class="px-5 py-4 border-b border-line">
                        <h2 class="text-base font-semibold text-ink">Informații canal</h2>
                    </div>
                    <div class="p-5 space-y-4">
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-muted">Channel ID</span>
                            <code class="text-sm font-mono bg-cream px-2 py-0.5 rounded text-inkSoft">{{ $channel->id }}</code>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-muted">Agent asociat</span>
                            <span class="text-sm font-medium text-ink">{{ $bot->name }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl border border-line shadow-sm">
                    <div class="px-5 py-4 border-b border-line">
                        <h2 class="text-base font-semibold text-ink">Temă widget</h2>
                        <p class="mt-1 text-xs text-muted">Alege o temă din catalog. Culorile, gradientul și forma bulei sunt presetate să arate coerent împreună.</p>
                    </div>
                    <div class="p-5">
                        ...
                        <div id="preset-grid" class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                            @foreach($themePresets as $preset)
                                <button type="button"
                                        ...
                                        class="preset-card group relative rounded-xl border-2 p-3 text-left transition-all hover:border-line focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-slate-400 {{ $activeTheme['preset'] === $preset['key'] ? 'border-slate-900 shadow-md' : 'border-line' }}"
                                        title="{{ $preset['description'] }}">
                                    <div class="h-8 rounded-md mb-2" style="background: linear-gradient(135deg, {{ $preset['accent'] }}, {{ $preset['accent_soft'] }})"></div>
                                    <div class="text-xs font-semibold text-ink leading-tight">{{ $preset['name'] }}</div>
                            ...
                            <button type="button"
                                    data-preset="custom"
                                    class="preset-card group relative rounded-xl border-2 border-dashed p-3 text-left transition-all hover:border-slate-400 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-slate-400 {{ $activeTheme['preset'] === 'custom' ? 'border-slate-900 shadow-md' : 'border-line' }}">
                                ...
                                <div class="text-xs font-semibold text-ink leading-tight">Custom</div>
                            </button>
                        </div>

                        {{-- Custom color picker (hidden unless preset=custom) --}}
                        <div id="custom-color-panel" class="mt-4 p-3 rounded-lg bg-cream border border-line {{ $activeTheme['preset'] === 'custom' ? '' : 'hidden' }}">
                            <label class="block text-xs font-medium text-muted mb-2">Culoare principală (hex)</label>
                            <div class="flex items-center gap-3">
                                <input type="color" id="custom-color-picker" value="{{ $activeTheme['accent'] }}"
                                       class="w-10 h-10 rounded-lg border border-line cursor-pointer p-0.5">
                                <input type="text" id="custom-color-text" value="{{ $activeTheme['accent'] }}"
                                       class="flex-1 rounded-lg border border-line px-3 py-2 text-sm font-mono text-inkSoft outline-none focus:border-slate-500">
                            </div>
                            <p class="mt-2 text-[11px] text-muted">Gradientul secundar se generează automat din culoarea aleasă.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl border border-line shadow-sm">
                    <div class="px-5 py-4 border-b border-line">
                        <h2 class="text-base font-semibold text-ink">Text și poziționare</h2>
                    </div>
                    <div class="p-5 space-y-5">
                        <div>
                            <label class="block text-sm font-medium text-inkSoft mb-1.5">Mesaj de întâmpinare</label>
                            <textarea name="greeting" id="widget-greeting" rows="2"
                                      class="w-full rounded-lg border border-line px-3 py-2 text-sm text-inkSoft focus:border-slate-500 focus:ring-1 focus:ring-slate-500 outline-none resize-none">{{ $channel->config['greeting'] ?? 'Bună! Cu ce te pot ajuta?' }}</textarea>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-inkSoft mb-1.5">Poziție widget</label>
                            <select name="position" id="widget-position"
                                    class="w-full rounded-lg border border-line px-3 py-2 text-sm text-inkSoft focus:border-slate-500 focus:ring-1 focus:ring-slate-500 outline-none">
                ...
                <div class="bg-white rounded-xl border border-line shadow-sm">
                    <div class="px-5 py-4 border-b border-line flex items-center justify-between">
                        <h2 class="text-base font-semibold text-ink">Cod de integrare</h2>
                    ...
                    <div class="p-5">
                        <p class="text-sm text-muted mb-3">Adaugă acest cod înainte de <code class="text-xs bg-cream px-1.5 py-0.5 rounded">&lt;/body&gt;</code> pe site-ul tău. Tema salvată aici se aplică automat — nu e nevoie să o duplici în atribute.</p>
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-xl border border-line shadow-sm sticky top-6">
                    <div class="px-5 py-4 border-b border-line">
                        <h2 class="text-base font-semibold text-ink">Previzualizare widget</h2>
                    </div>
                    <div class="p-5">
                        <div id="preview-container" class="relative bg-cream rounded-lg overflow-hidden" style="min-height: 560px;">
                            <div class="p-6">
                                <div class="h-4 bg-sand rounded w-3/4 mb-3"></div>
                                <div class="h-4 bg-sand rounded w-1/2 mb-3"></div>
                                <div class="h-4 bg-sand rounded w-2/3 mb-6"></div>
                                <div class="h-24 bg-sand rounded mb-4"></div>
                                <div class="h-4 bg-sand rounded w-5/6 mb-3"></div>
                                <div class="h-4 bg-sand rounded w-3/4"></div>
                            </div>

                            <div id="preview-window" class="absolute bottom-16 right-4 w-[300px] bg-white rounded-xl shadow-xl border border-line overflow-hidden">
                                ...
                                <div class="p-3 bg-cream space-y-2" style="min-height: 160px;">
                                    <div id="preview-greeting" class="bg-white border border-line rounded-xl rounded-bl-sm px-3 py-2 text-xs text-inkSoft max-w-[80%]">
                                    ...
                                    <div class="bg-white border border-line rounded-xl rounded-bl-sm px-3 py-2 text-xs text-inkSoft max-w-[80%]">
                                        Sigur! Cu ce anume te pot ajuta?
                                    </div>
                                </div>
                                <div class="flex items-center gap-2 p-2.5 border-t border-line">
                                    <div class="flex-1 bg-cream border border-line rounded-lg px-3 py-2 text-xs text-muted">Scrie un mesaj...</div>
...
(function() {
    ...
    function selectPreset(card) {
        ...
        card.classList.remove('border-line', 'border-line');

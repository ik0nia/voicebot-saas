...
<div>
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-base font-semibold text-ink">Starea botilor</h3>
        <a href="/dashboard/boti" class="text-xs font-medium text-coralh hover:underline">Gestioneaza &rarr;</a>
    </div>
    <div class="grid grid-cols-1 gap-4 {{ $botHealth->count() > 1 ? 'lg:grid-cols-2' : '' }}">
        ...
        @php
            ...
            $bot = $bh['bot'];
            $scoreColor = $score >= 80 ? 'text-emerald-600' : ($score >= 50 ? 'text-amber-600' : 'text-coral');
            ...
            <div class="p-5">
                <div class="flex items-start justify-between">
                    <div class="flex items-center gap-3.5 min-w-0">
                        ...
                        <div class="min-w-0">
                            <h4 class="text-sm font-bold text-ink truncate">{{ $bot->name }}</h4>
                            <div class="flex items-center gap-2 mt-1">
                                <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold {{ $bot->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-cream text-muted' }}">{{ $bot->is_active ? 'Activ' : 'Inactiv' }}</span>
                            </div>
                        </div>
                    </div>
                    <a href="/dashboard/boti/{{ $bot->id }}" class="shrink-0 p-1.5 rounded-lg text-muted hover:text-coral hover:bg-coralsoft transition-colors" title="Vezi detalii">
                ...
                <div class="mt-4 flex items-center gap-2.5 px-3.5 py-2.5 rounded-lg bg-white/80 border border-line/50 shadow-sm">
                    <div class="flex items-center justify-center w-8 h-8 rounded-full bg-coralsoft">
                    ...
                    <div>
                        <span class="text-lg font-bold text-ink">{{ $bh['recent_conversations'] }}</span>
                        <span class="text-xs text-muted ml-1">conversatii in ultimele 7 zile</span>
                    </div>
                </div>
                ...
                <div class="mt-3.5 grid grid-cols-2 gap-2">
                    <div class="flex items-center gap-2 text-xs">
                        ...
                        @else
                            <svg class="w-3.5 h-3.5 text-coral" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"/></svg>
                        @endif
                    ...
                    <div class="flex items-center gap-2 text-xs">
                        ...
                        @else
                            <svg class="w-3.5 h-3.5 text-coral" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"/></svg>
                        @endif
                        <span class="text-muted">Knowledge ({{ $bh['kb_ready'] }}/{{ $bh['kb_total'] }})</span>
                        @if($bh['kb_failed'] > 0)
                            <span class="text-[10px] text-coral font-semibold">{{ $bh['kb_failed'] }} esuate</span>
                        @endif
                        @if($bh['kb_pending'] > 0)
                    ...
                    <div class="flex items-center gap-2 text-xs">
                        ...
                        @else
                            <svg class="w-3.5 h-3.5 text-coral" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"/></svg>
                        @endif
                        <span class="text-muted">{{ $bh['active_channels'] }} canal{{ $bh['active_channels'] !== 1 ? 'e' : '' }} activ{{ $bh['active_channels'] !== 1 ? 'e' : '' }}</span>
                    </div>
                </div>
                ...
                <div class="mt-3.5 space-y-1.5">
                    ...
                    <div class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg {{ $score >= 50 ? 'bg-amber-50 border border-amber-200/50' : 'bg-coralsoft border border-coral/30/50' }}">
                        <svg class="w-3 h-3 shrink-0 {{ $score >= 50 ? 'text-amber-500' : 'text-coral' }}" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.345 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"/></svg>
                        <p class="text-[11px] font-medium {{ $score >= 50 ? 'text-amber-700' : 'text-coralh' }}">{{ $issue }}</p>
                    </div>
                    @endforeach
                </div>
                @endif

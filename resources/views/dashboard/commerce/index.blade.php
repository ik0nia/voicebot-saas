...
@section('breadcrumb')
<span class="text-ink font-medium">Conversii</span>
@endsection
...
<div class="space-y-6">
    ...
    <form method="GET" class="flex gap-3 items-end">
        <div>
            ...
            <input type="date" name="from" value="{{ $from }}" class="block mt-1 border-line rounded-lg text-sm">
        </div>
        <div>
            <label class="text-xs text-muted">Până la</label>
            <input type="date" name="to" value="{{ $to }}" class="block mt-1 border-line rounded-lg text-sm">
        </div>
    ...
    <div class="bg-white rounded-xl border border-line p-6">
        <h3 class="font-semibold text-ink mb-4">Funnel Conversie</h3>
        @php
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center gap-3">
                <div class="w-36 text-sm text-muted">{{ $bar['label'] }}</div>
                <div class="flex-1 bg-cream rounded-full h-6 overflow-hidden">
                    <div class="{{ $bar['color'] }} h-full rounded-full transition-all" style="width: {{ ($bar['value'] / $max) * 100 }}%"></div>
                </div>
                <div class="w-20 text-right text-sm font-semibold text-ink">{{ number_format($bar['value']) }}</div>
                <div class="w-16 text-right text-xs text-muted">{{ $max > 0 ? round(($bar['value'] / $max) * 100, 1) : 0 }}%</div>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-white rounded-xl border border-line p-5">
            <div class="text-sm text-muted">Venit Total Atribuit</div>
            <div class="text-3xl font-bold text-green-600">{{ number_format($totalRevenue / 100, 2) }} RON</div>
        </div>
        <div class="bg-white rounded-xl border border-line p-5">
            <div class="text-sm text-muted">Leads → Clienți</div>
            <div class="text-2xl font-bold text-blue-600">{{ $leadStats['converted_leads'] }} / {{ $leadStats['total_leads'] }}</div>
            <div class="text-xs text-muted">{{ $leadStats['total_leads'] > 0 ? round(($leadStats['converted_leads'] / $leadStats['total_leads']) * 100) : 0 }}% rată conversie</div>
        </div>
        <div class="bg-white rounded-xl border border-line p-5">
            <div class="text-sm text-muted">Oportunități active</div>
    ...
    <div class="bg-white rounded-xl border border-line p-5">
        <h3 class="font-semibold text-ink mb-3">Atribuire Conversii</h3>
        <table class="w-full text-sm">
            <thead><tr><th class="text-left py-2 text-muted">Tip</th><th class="text-right py-2 text-muted">Conversii</th><th class="text-right py-2 text-muted">Revenue</th></tr></thead>
            <tbody>
    ...
    <div class="bg-white rounded-xl border border-line p-5">
        <h3 class="font-semibold text-ink mb-3">Top Căutări Fără Rezultat</h3>
        <div class="space-y-1">
            @foreach($failedSearches as $fs)
            <div class="flex justify-between text-sm py-1 border-b border-slate-50">
                <span class="text-inkSoft">"{{ $fs->query }}"</span>
                <span class="text-muted">{{ $fs->cnt }}×</span>
            </div>
            @endforeach
        </div>
    </div>

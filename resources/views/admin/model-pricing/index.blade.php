...
@section('breadcrumb')<span class="text-ink font-medium">Prețuri Modele AI</span>@endsection
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-ink">Prețuri Modele AI</h1>
                <p class="text-sm text-muted">Costuri per model — tokens, minute, caractere</p>
            </div>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-line">
            <thead class="bg-cream">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold text-muted uppercase">Provider</th>
                    <th class="px-4 py-3 text-center text-xs font-semibold text-muted uppercase">Unitate</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold text-muted uppercase">Cost Input</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold text-muted uppercase">Cost Output</th>
                    <th class="px-4 py-3 text-right text-xs font-semibold text-muted uppercase">Context Max</th>
            ...
            <tbody class="divide-y divide-line">
                ...
                <tr class="{{ !$model->is_active ? 'opacity-50' : '' }}">
                    <form method="POST" action="{{ route('admin.model-pricing.update', $model) }}">
                        @csrf @method('PUT')
                        <td class="px-4 py-3">
                            <span class="font-mono text-sm font-medium text-ink">{{ $model->model_id }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $providerColors[$model->provider] ?? 'bg-cream text-inkSoft' }}">
                                {{ ucfirst($model->provider) }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-center">
                            <select name="pricing_unit" class="text-xs border border-line rounded-md px-2 py-1 focus:ring-coral focus:border-coral">
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                                ...
                                <input type="number" step="0.0001" name="input_cost" value="{{ $model->input_cost }}"
                                       class="w-24 text-right text-sm border border-line rounded-md px-2 py-1 focus:ring-coral focus:border-coral">
                            </div>
                        </td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex items-center justify-end gap-1">
                        ...
                        <td class="px-4 py-3 text-right">
                            <input type="number" name="max_context_tokens" value="{{ $model->max_context_tokens }}"
                                   class="w-28 text-right text-sm border border-line rounded-md px-2 py-1 focus:ring-coral focus:border-coral">
                        </td>
                        <td class="px-4 py-3 text-center">
                ...
                @empty
                <tr>
                    <td colspan="8" class="px-4 py-8 text-center text-muted">Nu există modele configurate.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="bg-white rounded-xl border border-line p-6">
        <h3 class="text-sm font-semibold text-ink mb-4">Adaugă model nou</h3>
        <form method="POST" action="{{ route('admin.model-pricing.store') }}" class="flex flex-wrap items-end gap-3">
            @csrf
            <div>
                <label class="block text-xs text-muted mb-1">Model ID</label>
                <input type="text" name="model_id" placeholder="gpt-4o-mini" required
                       class="w-48 text-sm border border-line rounded-md px-3 py-2 focus:ring-coral focus:border-coral">
            </div>
            <div>
                <label class="block text-xs text-muted mb-1">Provider</label>
                <select name="provider" class="text-sm border border-line rounded-md px-3 py-2 focus:ring-coral focus:border-coral">
            ...
            <div>
                <label class="block text-xs text-muted mb-1">Unitate preț</label>
                <select name="pricing_unit" class="text-sm border border-line rounded-md px-3 py-2 focus:ring-coral focus:border-coral">
                    @foreach($unitLabels as $key => $label)
            ...
            <div>
                <label class="block text-xs text-muted mb-1">Cost Input ($)</label>
                <input type="number" step="0.0001" name="input_cost" placeholder="0.15" required
                       class="w-24 text-sm border border-line rounded-md px-3 py-2 focus:ring-coral focus:border-coral">
            </div>
            <div>
                <label class="block text-xs text-muted mb-1">Cost Output ($)</label>
                <input type="number" step="0.0001" name="output_cost" placeholder="0.60" required
                       class="w-24 text-sm border border-line rounded-md px-3 py-2 focus:ring-coral focus:border-coral">
            </div>
            <div>
                <label class="block text-xs text-muted mb-1">Context Max</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl border border-line p-5">
            <div class="flex items-center gap-2 mb-3">
                ...
                <h3 class="text-sm font-bold text-ink">1 minut voce</h3>
                <span class="text-xs text-muted">(fără clonare)</span>
            </div>
            @if($voiceSource === 'real')
            ...
            @endif
            <div class="pt-2 border-t border-line flex justify-between items-baseline">
                <span class="text-sm font-semibold text-ink">Cost / minut</span>
                <span class="text-lg font-bold text-blue-600">${{ number_format($voiceTotal, 3) }}</span>
            </div>
            <div class="mt-2 space-y-1 text-xs text-muted">
                <div class="flex justify-between"><span>Apel 3 min</span><span class="font-mono font-medium text-inkSoft">${{ number_format($voiceTotal * 3, 2) }}</span></div>
                <div class="flex justify-between"><span>Apel 5 min</span><span class="font-mono font-medium text-inkSoft">${{ number_format($voiceTotal * 5, 2) }}</span></div>
                <div class="flex justify-between"><span>Apel 10 min</span><span class="font-mono font-medium text-inkSoft">${{ number_format($voiceTotal * 10, 2) }}</span></div>
                <div class="flex justify-between border-t border-line pt-1"><span>100 apeluri/lună (5 min)</span><span class="font-mono font-semibold text-ink">${{ number_format($voiceTotal * 5 * 100, 0) }}</span></div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-line p-5">
            ...
            <div class="mt-2 space-y-1 text-xs text-muted">
                <div class="flex justify-between"><span>Apel 3 min</span><span class="font-mono font-medium text-inkSoft">${{ number_format($voiceClonedTotal * 3, 2) }}</span></div>
                <div class="flex justify-between"><span>Apel 5 min</span><span class="font-mono font-medium text-inkSoft">${{ number_format($voiceClonedTotal * 5, 2) }}</span></div>
                <div class="flex justify-between"><span>Apel 10 min</span><span class="font-mono font-medium text-inkSoft">${{ number_format($voiceClonedTotal * 10, 2) }}</span></div>
                <div class="flex justify-between border-t border-line pt-1"><span>100 apeluri/lună (5 min)</span><span class="font-mono font-semibold text-ink">${{ number_format($voiceClonedTotal * 5 * 100, 0) }}</span></div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-line p-5">
            <div class="flex items-center gap-2 mb-3">
                ...
                <h3 class="text-sm font-bold text-ink">Conversație chat</h3>
            </div>
            @if($chatSource === 'real')
            ...
            @if($chatModelBreakdown->isNotEmpty())
                <div class="space-y-1.5 text-xs text-muted mb-3">
                    @foreach($chatModelBreakdown as $m)
                    <div class="flex justify-between">
                        <span>{{ $m->model }} <span class="text-muted">({{ number_format($m->total_calls) }}x)</span></span>
            ...
            @endif
            <div class="pt-2 border-t border-line flex justify-between items-baseline">
                <span class="text-sm font-semibold text-ink">Cost / conversație</span>
                <span class="text-lg font-bold text-emerald-600">${{ number_format($chatTotal, 4) }}</span>
            </div>
            <div class="mt-2 space-y-1 text-xs text-muted">
                <div class="flex justify-between"><span>50 conv/zi</span><span class="font-mono font-medium text-inkSoft">${{ number_format($chatTotal * 50, 2) }}/zi</span></div>
                <div class="flex justify-between"><span>200 conv/zi</span><span class="font-mono font-medium text-inkSoft">${{ number_format($chatTotal * 200, 2) }}/zi</span></div>
                <div class="flex justify-between border-t border-line pt-1"><span>1000 conv/lună</span><span class="font-mono font-semibold text-ink">${{ number_format($chatTotal * 1000, 2) }}</span></div>
            </div>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-line">
            <thead class="bg-cream">
                <tr>
                    ...
                    <th class="px-4 py-2 text-right text-xs font-semibold text-muted">Apeluri</th>
                    <th class="px-4 py-2 text-right text-xs font-semibold text-muted">Cost</th>
                    <th class="px-4 py-2 text-right text-xs font-semibold text-muted">Timp mediu</th>
            ...
            <tbody class="divide-y divide-line">
                ...
                <tr>
                    ...
                    <td class="px-4 py-2 text-sm text-right {{ $m->errors > 0 ? 'text-coral font-medium' : 'text-muted' }}">{{ $m->errors }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>

...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-ink">Costuri & Profitabilitate</h1>
...
<form method="GET" class="mb-4 bg-white rounded-xl border border-line p-4 flex flex-wrap items-end gap-3">
    ...
    @if($mode === 'bot' && $tenantFilter)
        <div>
            <label class="block text-xs font-medium text-muted mb-1">Agent AI</label>
            <select name="bot_id" class="rounded-lg border border-line px-3 py-2 text-sm">
                <option value="">— Toți —</option>
                @foreach($bots as $b)
...
<div class="mb-4 flex flex-wrap items-center gap-2 text-xs">
    <a href="{{ route('admin.costs.index', array_merge(request()->except('setup_ai_only', 'page'), ['setup_ai_only' => 1])) }}"
       class="inline-flex items-center gap-1.5 rounded-full border border-coral bg-coralsoft px-3 py-1 font-medium text-coralh hover:bg-coralsoft">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"/></svg>
        Doar setup AI
    </a>
    <span class="text-muted">Afișează activitatea AI de configurare a agenților per-metric, pentru investigare de abuzuri.</span>
</div>

{{-- KPI-uri top --}}
<div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
    <div class="bg-white rounded-xl border border-line p-4">
        <p class="text-xs text-muted">Cost total</p>
        <p class="text-lg font-bold text-ink mt-1">${{ number_format(($total['grand_total']+$setupAiTotals['cost_cents'])/100, 2) }}</p>
        <p class="text-xs text-muted mt-0.5">≈ {{ number_format(($total['grand_total']+$setupAiTotals['cost_cents'])/100*$usdRon, 2) }} lei</p>
    </div>
    <div class="bg-white rounded-xl border border-line p-4">
        <p class="text-xs text-muted">Voice (apeluri)</p>
        <p class="text-lg font-bold text-ink mt-1">${{ number_format($total['voice_total']/100, 2) }}</p>
        <p class="text-xs text-muted mt-0.5">
            {{ $total['calls'] }} apeluri &middot; {{ number_format($total['seconds']/60, 1) }} min
        </p>
    </div>
    <div class="bg-white rounded-xl border border-line p-4">
        <p class="text-xs text-muted">Chat</p>
        <p class="text-lg font-bold text-ink mt-1">${{ number_format($total['chat_cost']/100, 2) }}</p>
        <p class="text-xs text-muted mt-0.5">
            {{ $total['conversations'] }} conv &middot; {{ $total['messages'] }} mesaje
        </p>
    </div>
    <div class="bg-white rounded-xl border border-line p-4">
        <p class="text-xs text-muted">Cost / min apel</p>
        @php $perMin = $total['seconds'] > 0 ? ($total['voice_total']/100) / ($total['seconds']/60) : 0; @endphp
        <p class="text-lg font-bold text-ink mt-1">${{ number_format($perMin, 3) }}</p>
        <p class="text-xs text-muted mt-0.5">≈ {{ number_format($perMin*$usdRon, 3) }} lei/min</p>
    </div>
    <div class="bg-white rounded-xl border border-line p-4">
        <p class="text-xs text-muted">Configurare agenți (AI)</p>
        <p class="text-lg font-bold text-ink mt-1">${{ number_format($setupAiTotals['cost_usd'], 4) }}</p>
        <p class="text-xs text-muted mt-0.5">
...
@if($total['voice_total'] > 0)
    <div class="bg-white rounded-xl border border-line p-4 mb-6">
        <p class="text-xs font-semibold text-muted uppercase mb-2">Defalcare voice</p>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
            <div>
                <p class="text-muted">OpenAI Realtime</p>
                <p class="font-mono font-semibold">${{ number_format($total['voice_openai']/100, 4) }}</p>
            </div>
            <div>
                <p class="text-muted">Twilio voice</p>
            ...
            @if($total['voice_unattributed'] > 0)
                <div title="Cost din apeluri vechi, înainte să tragem breakdown-ul OpenAI/Twilio. Apelurile noi au defalcare completă.">
                    <p class="text-muted">Istoric (neatribuit)</p>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-line">
            @forelse($grouped as $r)
                ...
                @endphp
                <tr class="hover:bg-cream {{ $highlight }}">
                    <td class="px-4 py-2 font-medium text-ink">
                        ...
                        @if($mode === 'tenant' && $setup30dRon > 10)
                            <span class="ml-1.5 inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold
                                {{ $setup30dRon > 50 ? 'bg-red-200 text-coralh' : 'bg-amber-200 text-amber-900' }}"
                    ...
                    @if($mode === 'bot')
                        <td class="px-4 py-2 text-muted text-xs">{{ $r['tenant_name'] }}</td>
                    @endif
                    <td class="px-4 py-2 text-right tabular-nums">{{ $r['calls'] }}</td>
                    <td class="px-4 py-2 text-right tabular-nums text-xs text-muted">{{ number_format($r['seconds']/60, 1) }}</td>
                    <td class="px-4 py-2 text-right font-mono">${{ number_format($r['voice']/100, 3) }}</td>
                    <td class="px-4 py-2 text-right tabular-nums text-xs text-muted">{{ $r['conversations'] }} / {{ $r['messages'] }}</td>
                        ...
                        <td class="px-4 py-2 text-right font-mono text-xs">
                            @if($setupCents > 0)
                                <a href="{{ route('admin.costs.index', array_merge(request()->except('page'), ['setup_ai_only' => 1, 'tenant_id' => $r['id']])) }}"
                                   class="text-coralh hover:underline">
                    ...
                    <td class="px-4 py-2 text-right font-mono font-semibold text-inkSoft">{{ number_format($totalCents/100*$usdRon, 2) }}</td>
                </tr>
            @empty
                @php
                ...
                @endphp
                <tr>
                    <td colspan="{{ $emptyCols }}" class="px-4 py-8 text-center text-sm text-muted">
                        Nicio activitate în perioada selectată. Rulează
                        <code class="bg-cream px-1.5 py-0.5 rounded">php artisan costs:rollup --from=... --to=...</code>
...
@if($platformTotal > 0)
    <div class="mt-6 bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-4 py-3 border-b border-line flex items-center justify-between">
            <div>
                <p class="text-xs font-semibold text-muted uppercase">Overhead platformă</p>
                <p class="text-xs text-muted mt-0.5">API-uri care nu se atribuie unui tenant / conversație: indexare KB, agent web-scan, imagini social, voice cloning.</p>
            </div>
            <div class="text-right">
                <p class="text-xs text-muted">Total</p>
                <p class="text-lg font-bold text-ink font-mono">${{ number_format($platformTotal/100, 4) }}</p>
                <p class="text-xs text-muted">≈ {{ number_format($platformTotal/100*$usdRon, 2) }} lei</p>
            </div>
        </div>
        <table class="w-full text-sm">
            <thead class="bg-cream text-muted text-xs uppercase">
            ...
            <tbody class="divide-y divide-line">
                @foreach($platform as $p)
                    <tr class="hover:bg-cream">
                        ...
                        <td class="px-4 py-2 text-xs text-muted">{{ $p->provider }} / {{ $p->model }}</td>
...
@if($topAbusers['by_cost']->isNotEmpty() || $topAbusers['by_count']->isNotEmpty())
    <div class="mt-6 bg-white rounded-xl border border-line overflow-hidden"
        ...
        <div class="px-4 py-3 border-b border-line">
            <p class="text-xs font-semibold text-muted uppercase">Setup AI — top consumatori (ultimele 30 zile)</p>
            <p class="text-xs text-muted mt-0.5">
                Fereastră: {{ $topAbusers['window_start']->format('d M') }} → {{ $topAbusers['window_end']->format('d M Y') }}.
                Limita curentă e persistată pe <code class="bg-cream px-1.5 py-0.5 rounded">tenant.settings.setup_ai_limit_ron</code>
        ...
        <div class="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-line">
            ...
            <div class="p-4">
                ...
                @if($topAbusers['by_cost']->isEmpty())
                    <p class="text-xs text-muted">Niciun tenant cu cost setup-AI &gt; 0 în perioada selectată.</p>
                @else
                    <table class="w-full text-sm">
                                ...
                                <tr class="border-t border-line">
                                    <td class="py-2 font-medium text-ink">
                                        ...
                                        @if($currentLimit !== null)
                                            <span class="ml-1 text-[10px] text-muted">(limită: {{ number_format((float)$currentLimit, 2) }} lei)</span>
                                        @endif
                                    </td>
                                    <td class="py-2 text-right font-mono">{{ number_format($row->cost_ron, 2) }} lei</td>
                                    <td class="py-2 text-right tabular-nums text-xs text-muted">{{ number_format($row->call_count) }}</td>
                                    <td class="py-2 text-right">
                                        <button type="button"
                                                @click="openTenantId = openTenantId === {{ $row->tenant_id }} ? null : {{ $row->tenant_id }}; limitValue = '{{ $currentLimit ?? '' }}'"
                                                class="text-xs rounded border border-line bg-white px-2 py-1 hover:bg-cream">
                                            Set limită
                                        </button>
                                    </td>
                                </tr>
                                <tr x-show="openTenantId === {{ $row->tenant_id }}" x-cloak>
                                    <td colspan="4" class="bg-cream px-3 py-3">
                                        <form method="POST" action="{{ route('admin.costs.setupAiLimit', ['tenantId' => $row->tenant_id]) }}" class="flex flex-wrap items-end gap-2">
                                            @csrf
                                            <div>
                                                <label class="block text-[10px] font-semibold text-muted uppercase mb-0.5">Max setup-AI RON / lună</label>
            ...
            {{-- By count --}}
            <div class="p-4">
                <p class="text-xs font-semibold text-inkSoft mb-2">Top 5 după număr de apeluri</p>
                @if($topAbusers['by_count']->isEmpty())
                    <p class="text-xs text-muted">Niciun tenant cu apeluri setup-AI în perioada selectată.</p>
                @else
                    <table class="w-full text-sm">
                        <thead class="text-xs uppercase text-muted">
                            <tr>
                        ...
                        <tbody>
                            @foreach($topAbusers['by_count'] as $row)
                                ...
                                <tr class="border-t border-line">
                                    <td class="py-2 font-medium text-ink">{{ $tenant?->name ?? '#'.$row->tenant_id }}</td>
                                    <td class="py-2 text-right tabular-nums">{{ number_format($row->call_count) }}</td>
                                    <td class="py-2 text-right font-mono text-xs text-muted">{{ number_format($row->cost_ron, 2) }} lei</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
...
@if(!empty($demoStats))
<div class="mt-8 bg-white rounded-xl border border-line p-5">
    <div class="flex items-center justify-between mb-4">
        <div>
            <h2 class="text-base font-bold text-ink">Demo pe landing pages</h2>
            <p class="text-xs text-muted mt-0.5">Ultimele 30 zile · Flag <code class="px-1 py-0.5 bg-cream rounded">NICHE_PUBLIC_DEMO_ENABLED</code></p>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5">
        <div class="bg-cream rounded-lg p-3">
            <div class="text-xs text-muted font-semibold uppercase tracking-wide">Demo-uri vizualizate</div>
            <div class="text-2xl font-bold text-ink mt-1">{{ number_format($demoStats['viewed']) }}</div>
        </div>
        <div class="bg-cream rounded-lg p-3">
            <div class="text-xs text-muted font-semibold uppercase tracking-wide">Mesaje trimise</div>
            <div class="text-2xl font-bold text-ink mt-1">{{ number_format($demoStats['messages']) }}</div>
        </div>
        <div class="bg-cream rounded-lg p-3">
            <div class="text-xs text-muted font-semibold uppercase tracking-wide">Calificate (≥3 mesaje)</div>
            <div class="text-2xl font-bold text-emerald-600 mt-1">{{ number_format($demoStats['qualified']) }}</div>
        </div>
        <div class="bg-cream rounded-lg p-3">
            <div class="text-xs text-muted font-semibold uppercase tracking-wide">Rată calificare</div>
            <div class="text-2xl font-bold text-ink mt-1">{{ $demoStats['rate'] }}%</div>
        </div>
        <div class="bg-cream rounded-lg p-3">
            <div class="text-xs text-muted font-semibold uppercase tracking-wide">Înregistrări (demo → signup)</div>
    ...
    @if(!empty($demoStats['by_niche']))
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="text-left text-xs font-semibold text-muted uppercase border-b border-line">
                ...
                <tbody>
                    @foreach(array_slice($demoStats['by_niche'], 0, 3) as $row)
                        <tr class="border-b border-line">
                            ...
                            <td class="px-3 py-2 text-right tabular-nums text-muted">{{ number_format($row['messages']) }}</td>
    ...
    @else
        <p class="text-xs text-muted">Încă niciun eveniment — ori flag-ul e oprit, ori landing-urile încă n-au primit trafic în ultimele 30 de zile.</p>
    @endif
</div>
@endif

{{-- Re-agregare manuală --}}
<form method="POST" action="{{ route('admin.costs.reaggregate') }}" class="mt-6 bg-cream rounded-xl border border-line p-4 flex flex-wrap items-end gap-3">
    @csrf
    <div>
        <p class="text-xs font-semibold text-muted uppercase">Re-agregare manuală</p>
        <p class="text-xs text-muted mt-0.5">Pentru backfill sau după webhook-uri întârziate.</p>
    </div>
    <div>
        <label class="block text-xs font-medium text-muted mb-1">De la</label>
        <input type="date" name="from" value="{{ $start->toDateString() }}" class="rounded-lg border border-line px-3 py-1.5 text-sm" required>
    </div>
    <div>
        <label class="block text-xs font-medium text-muted mb-1">Până la</label>
        <input type="date" name="to" value="{{ $end->toDateString() }}" class="rounded-lg border border-line px-3 py-1.5 text-sm" required>
    </div>

...
<div class="mb-6 flex items-start justify-between gap-4">
    <div>
        <h1 class="text-2xl font-bold text-ink">Setup AI — detaliat</h1>
    ...
    <a href="{{ route('admin.costs.index', request()->except('setup_ai_only', 'page')) }}"
       class="inline-flex items-center gap-1.5 rounded-lg border border-line bg-white px-3 py-2 text-xs font-medium text-inkSoft hover:bg-cream">
        ← Înapoi la sumar
    </a>
</div>
...
<form method="GET" class="mb-4 bg-white rounded-xl border border-line p-4 flex flex-wrap items-end gap-3">
    ...
    @if($botFilter)
        <div>
            <label class="block text-xs font-medium text-muted mb-1">Bot ID</label>
            <input type="number" name="bot_id" value="{{ $botFilter }}" class="rounded-lg border border-line px-3 py-2 text-sm w-24">
        </div>
    @endif
    @if($userFilter)
        <div>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-xl border border-line p-4">
        <p class="text-xs text-muted">Apeluri</p>
        <p class="text-lg font-bold text-ink mt-1">{{ number_format($totals['call_count']) }}</p>
    </div>
    <div class="bg-white rounded-xl border border-line p-4">
        <p class="text-xs text-muted">Cost USD</p>
        <p class="text-lg font-bold text-ink mt-1">${{ number_format($totals['cost_usd'], 4) }}</p>
    </div>
    <div class="bg-white rounded-xl border border-line p-4">
        <p class="text-xs text-muted">Cost RON</p>
        <p class="text-lg font-bold text-ink mt-1">{{ number_format($totals['cost_ron'], 2) }} lei</p>
    </div>
    <div class="bg-white rounded-xl border border-line p-4">
        <p class="text-xs text-muted">Tokens in / out</p>
        <p class="text-lg font-bold text-ink mt-1">{{ number_format($totals['tokens_in']) }} / {{ number_format($totals['tokens_out']) }}</p>
    </div>
</div>
        ...
        <tbody class="divide-y divide-line">
            @forelse($rows as $row)
                ...
                <tr class="hover:bg-cream">
                    <td class="px-3 py-2 text-xs text-muted tabular-nums whitespace-nowrap">{{ $row->created_at->format('d M H:i:s') }}</td>
                    <td class="px-3 py-2 text-ink">{{ $row->tenant?->name ?? '#'.$row->tenant_id }}</td>
                    <td class="px-3 py-2">
                        @if($row->bot)
                            <span class="text-ink">{{ $row->bot->name }}</span>
                            <span class="text-xs text-muted">#{{ $row->bot->id }}</span>
                        @else
                    ...
                    <td class="px-3 py-2 text-xs">
                        @if($row->user)
                            <span class="text-ink">{{ $row->user->name ?: $row->user->email }}</span>
                            <span class="block text-[10px] text-muted">#{{ $row->user->id }}</span>
                        @else
                    ...
                    <td class="px-3 py-2 text-xs">
                        @if($target)
                            <span class="inline-flex items-center rounded-full bg-cream px-2 py-0.5 font-mono text-[10px] text-inkSoft">{{ $target }}</span>
                        @else
                            <span class="text-muted">—</span>
                        @endif
                    </td>
                    <td class="px-3 py-2 text-xs text-muted font-mono">{{ $row->model }}</td>
                    <td class="px-3 py-2 text-right text-xs tabular-nums text-muted">

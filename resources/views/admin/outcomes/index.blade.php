...
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-900">Venituri & outcomes</h1>
...
<form method="GET" class="mb-4 bg-white rounded-xl border border-slate-200 p-4 flex flex-wrap items-end gap-3">
    ...
    <button type="submit" class="rounded-lg bg-red-700 px-4 py-2 text-sm font-semibold text-white hover:bg-red-800">Aplică</button>
</form>

{{-- Spend vs value card --}}
<div class="bg-white rounded-xl border border-slate-200 p-5 mb-4">
    <p class="text-xs font-semibold text-slate-600 uppercase mb-3">Cheltuit vs încasat</p>
    <div class="grid grid-cols-3 gap-4">
        <div>
            <p class="text-xs text-slate-500">Cheltuit (AI + telefonie)</p>
            <p class="text-xl font-bold text-slate-900 mt-1 font-mono">{{ number_format($spendCents/100, 2) }} $</p>
            <p class="text-xs text-slate-500">≈ {{ number_format($spendRon, 2) }} lei</p>
        </div>
        <div>
            <p class="text-xs text-slate-500">Încasat (venit atribuit)</p>
            <p class="text-xl font-bold text-emerald-700 mt-1 font-mono">{{ number_format($total['revenue_booked_cents']/100, 2) }} $</p>
            <p class="text-xs text-slate-500">≈ {{ number_format($revenueRon, 2) }} lei</p>
        </div>
        <div>
            <p class="text-xs text-slate-500">Raport venit / cost</p>
...
<div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <p class="text-xs text-slate-500 uppercase font-semibold">Programări</p>
        <p class="text-2xl font-bold text-slate-900 mt-1">{{ $total['bookings_requested'] }}</p>
        <p class="text-xs text-emerald-600">{{ $total['bookings_confirmed'] }} confirmate</p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <p class="text-xs text-slate-500 uppercase font-semibold">Lead-uri</p>
        <p class="text-2xl font-bold text-slate-900 mt-1">{{ $total['leads_generated'] }}</p>
        <p class="text-xs text-slate-500">{{ $total['callbacks_requested'] }} callback-uri</p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <p class="text-xs text-slate-500 uppercase font-semibold">Comenzi influențate</p>
        <p class="text-2xl font-bold text-slate-900 mt-1">{{ $total['orders_influenced'] }}</p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <p class="text-xs text-slate-500 uppercase font-semibold">Conversații</p>
        <p class="text-2xl font-bold text-slate-900 mt-1">{{ $total['conversations_count'] }}</p>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <p class="text-xs text-slate-500 uppercase font-semibold">Apeluri voce</p>
        <p class="text-2xl font-bold text-slate-900 mt-1">{{ $total['voice_calls_count'] }}</p>
    </div>
</div>
        ...
        <tbody class="divide-y divide-slate-100">
            @forelse($grouped as $r)
                <tr class="hover:bg-slate-50">
                    ...
                    @if($mode === 'bot')<td class="px-4 py-2 text-xs text-slate-600">{{ $r['tenant_name'] }}</td>@endif
                    <td class="px-4 py-2 text-right tabular-nums">{{ $r['bookings_requested'] }} <span class="text-xs text-slate-400">/ {{ $r['bookings_confirmed'] }}</span></td>
                    ...
                    <td class="px-4 py-2 text-right tabular-nums text-xs text-slate-500">{{ $r['conversations_count'] }}</td>
                </tr>
            @empty
                <tr><td colspan="{{ $mode === 'bot' ? 7 : 6 }}" class="px-4 py-8 text-center text-sm text-slate-500">
                    Nicio activitate pentru perioada selectată. Rulează <code class="bg-slate-100 px-1.5 py-0.5 rounded">php artisan outcomes:rollup --from=... --to=...</code>.
                </td></tr>
            @endforelse
        </tbody>
    </table>

...
@section('breadcrumb')<span class="text-ink font-medium">Lead-uri SaaS</span>@endsection

@section('content')
<div class="mb-6 flex flex-wrap items-center justify-between gap-3">
    <div>
        <h1 class="text-2xl font-bold text-ink">Lead-uri SaaS</h1>
        <p class="text-sm text-muted mt-1">Toate mesajele venite prin formularul de contact + landing page-urile de nișă.</p>
    </div>
    <div class="flex items-center gap-2 text-sm">
        <span class="text-muted">Total:</span>
        <span class="font-semibold text-ink">{{ $counts['total'] }}</span>
        <span class="ml-3 px-2 py-0.5 rounded-full bg-coralsoft text-coralh text-xs font-semibold">{{ $counts['new'] }} noi</span>
...
<form method="GET" class="mb-4 bg-white rounded-xl border border-line p-4 flex flex-wrap items-end gap-3">
    <div class="flex-1 min-w-[200px]">
        <label class="block text-xs font-medium text-muted mb-1">Caută</label>
    ...
    <div>
        <label class="block text-xs font-medium text-muted mb-1">Sursă</label>
        <select name="source" class="rounded-lg border border-line px-3 py-2 text-sm">
            <option value="">— Toate —</option>
    ...
    @if($q || $status || $source)
        <a href="{{ route('admin.leads.index') }}" class="px-3 py-2 text-sm text-muted hover:text-ink">Reset</a>
    @endif
</form>
        ...
        <tbody class="divide-y divide-line">
            @forelse($leads as $lead)
                <tr class="hover:bg-cream">
                    ...
                    <td class="px-4 py-2 font-medium text-ink">{{ $lead->name }}</td>
                    <td class="px-4 py-2"><a href="mailto:{{ $lead->email }}" class="text-coralh hover:underline">{{ $lead->email }}</a></td>
                    <td class="px-4 py-2 text-muted">{{ $lead->company ?: '—' }}</td>
                    <td class="px-4 py-2">
                    ...
                    <td class="px-4 py-2">
                        @php $badge = ['new'=>'bg-coralsoft text-coralh','contacted'=>'bg-amber-100 text-amber-800','qualified'=>'bg-indigo-100 text-indigo-800','disqualified'=>'bg-cream text-muted','converted'=>'bg-emerald-100 text-emerald-800'][$lead->status] ?? 'bg-cream'; @endphp
                        <span class="inline-flex px-2 py-0.5 rounded {{ $badge }} text-xs font-semibold">{{ $lead->status }}</span>
                    </td>
                    <td class="px-4 py-2 text-right">
                        <a href="{{ route('admin.leads.show', $lead) }}" class="text-coralh hover:underline text-sm font-medium">Deschide →</a>
                    </td>
                </tr>
            @empty
                <tr><td colspan="8" class="px-4 py-12 text-center text-sm text-muted">Niciun lead încă.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>

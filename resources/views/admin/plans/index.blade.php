...
@section('breadcrumb')<span class="text-ink font-medium">Pachete & Prețuri</span>@endsection
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-bold text-ink">Pachete & Prețuri</h1>
    ...
    @foreach(['webchat', 'voice', 'bundle'] as $type)
        ...
        <div>
            <div class="flex items-center gap-2 mb-3">
                ...
                <h2 class="text-sm font-bold text-ink uppercase tracking-wide">{{ $info['label'] }}</h2>
                <span class="text-xs text-muted">({{ $typePlans->count() }} pachete)</span>
            </div>

            @if($typePlans->isEmpty())
                <div class="bg-white rounded-xl border border-line p-6 text-center">
                    <p class="text-sm text-muted">Nu există pachete de tip {{ $info['label'] }}.</p>
                </div>
            @else
                    ...
                    @foreach($typePlans as $plan)
                        <div class="bg-white rounded-xl border {{ $plan->is_popular ? 'border-coral/40 ring-2 ring-red-100' : 'border-line' }} p-5 relative {{ !$plan->is_active ? 'opacity-60' : '' }}">
                            ...
                            <div class="absolute -top-2.5 left-4 flex gap-1">
                                @if($plan->is_popular)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-coral text-white">Popular</span>
                                @endif
                                @if($plan->tenant_id)
                            ...
                            <div class="flex items-start justify-between mb-3">
                                <div>
                                    ...
                                    <span class="text-xs font-mono text-muted">{{ $plan->slug }}</span>
                                </div>
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <form method="POST" action="{{ route('admin.plans.update', $plan) }}" class="inline">
                                        ...
                                        <button type="submit" title="{{ $plan->is_active ? 'Dezactivează' : 'Activează' }}">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium {{ $plan->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-cream text-muted' }}">
                            ...
                            <div class="flex items-baseline gap-2 mb-3">
                                <span class="text-2xl font-bold text-ink">{{ number_format($plan->price_monthly, 0) }} <span class="text-sm font-normal text-muted">lei/lună</span></span>
                                @if($plan->price_yearly > 0)
                                    <span class="text-xs text-muted">/ {{ number_format($plan->price_yearly, 0) }} lei/an</span>
                                @endif
                            </div>
                            ...
                            @if(is_array($plan->limits) && count($plan->limits) > 0)
                                <div class="mb-3 space-y-1">
                                    <p class="text-xs font-semibold text-muted uppercase">Limite</p>
                                    @php
                                        $limitLabels = [
                                            'bots' => 'Boți',
                                    ...
                                    @foreach($plan->limits as $key => $value)
                                        <div class="flex justify-between text-xs">
                                            <span class="text-muted">{{ $limitLabels[$key] ?? $key }}</span>
                                            <span class="font-medium text-inkSoft">
                                                @if(is_array($value))
                                                    {{ implode(', ', $value) }}
                                                @elseif($value == -1)
                                                    Nelimitat
                            ...
                            @if(is_array($plan->overage) && count($plan->overage) > 0)
                                <div class="mb-3 space-y-1">
                                    <p class="text-xs font-semibold text-muted uppercase">Overage</p>
                                    @php
                                        $overageLabels = [
                                    ...
                                    @foreach($plan->overage as $key => $value)
                                        <div class="flex justify-between text-xs">
                                            <span class="text-muted">{{ $overageLabels[$key] ?? $key }}</span>
                                            <span class="font-mono font-medium text-inkSoft">{{ number_format($value, 4) }} RON</span>
                                        </div>
                                    @endforeach
                                </div>
                            @endif
                            ...
                            <div class="flex items-center gap-2 pt-3 border-t border-line">
                                <a href="{{ route('admin.plans.edit', $plan) }}" class="flex-1 text-center px-3 py-1.5 text-sm font-medium text-coral hover:text-coralh bg-coralsoft hover:bg-coralsoft rounded-lg transition-colors">
                                    Editează
                                </a>
                                <form method="POST" action="{{ route('admin.plans.destroy', $plan) }}" class="flex-1"
                                      onsubmit="return confirm('Ștergi pachetul {{ $plan->name }}?')">
                                    @csrf @method('DELETE')
                                    <button type="submit" class="w-full px-3 py-1.5 text-sm font-medium text-muted hover:text-coral bg-cream hover:bg-cream rounded-lg transition-colors">
                                        Șterge
                                    </button>
                                </form>
                            </div>

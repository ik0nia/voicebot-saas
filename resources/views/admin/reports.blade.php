...
<div class="space-y-8">
    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-ink">Rapoarte</h1>
                <p class="text-sm text-muted mt-0.5">Analiză detaliată a platformei &mdash; generat {{ $now->format('d/m/Y H:i') }}</p>
            </div>
        </div>
    ...
    <div class="bg-gradient-to-r from-slate-800 to-slate-900 rounded-2xl overflow-hidden shadow-lg">
        <div class="px-6 py-4 flex items-center justify-between">
            ...
            <span class="text-xs text-muted">7 zile</span>
        </div>
        <div class="px-6 pb-5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-{{ min(count($latencyBreakdown['per_model']), 5) }} gap-3 mb-4">
                ...
                <div class="bg-white/10 backdrop-blur rounded-xl p-4">
                    ...
                    <p class="text-[11px] text-muted mt-1">{{ number_format($lm['cnt']) }} cereri</p>
                </div>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <div>
                    <p class="text-xs font-medium text-muted mb-2">Trend orar (24h)</p>
                    <div class="flex items-end gap-px h-14 bg-white/5 rounded-lg p-1.5">
                        @foreach($latencyBreakdown['hourly_trend'] as $ht)
                            ...
                            <div class="flex-1 {{ $barColor }} rounded-t group relative cursor-pointer" style="height: {{ max($pct, 3) }}%">
                                <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-white text-ink text-[10px] font-medium rounded px-2 py-1 whitespace-nowrap z-10 shadow-lg">
                ...
                <div>
                    <p class="text-xs font-medium text-muted mb-2">Distribuție</p>
                    @php
                    ...
                    <div class="flex h-8 rounded-lg overflow-hidden">
                        @if($fastPct > 0)<div class="bg-emerald-400 relative group cursor-pointer" style="width: {{ $fastPct }}%"><div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-white text-ink text-[10px] font-medium rounded px-2 py-1 whitespace-nowrap z-10 shadow-lg">Rapid: {{ number_format($cats['fast']) }} ({{ $fastPct }}%)</div></div>@endif
                        @if($normalPct > 0)<div class="bg-cyan-400 relative group cursor-pointer" style="width: {{ $normalPct }}%"><div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-white text-ink text-[10px] font-medium rounded px-2 py-1 whitespace-nowrap z-10 shadow-lg">Normal: {{ number_format($cats['normal']) }} ({{ $normalPct }}%)</div></div>@endif
                        @if($slowPct > 0)<div class="bg-amber-400 relative group cursor-pointer" style="width: {{ $slowPct }}%"><div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-white text-ink text-[10px] font-medium rounded px-2 py-1 whitespace-nowrap z-10 shadow-lg">Lent: {{ number_format($cats['slow']) }} ({{ $slowPct }}%)</div></div>@endif
                        @if($verySlowPct > 0)<div class="bg-red-400 relative group cursor-pointer" style="width: {{ $verySlowPct }}%"><div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block bg-white text-ink text-[10px] font-medium rounded px-2 py-1 whitespace-nowrap z-10 shadow-lg">F. Lent: {{ number_format($cats['very_slow']) }} ({{ $verySlowPct }}%)</div></div>@endif
                    </div>
    ...
    {{-- TAB NAVIGATION --}}
    {{-- ============================================================= --}}
    <div class="bg-white rounded-2xl border border-line shadow-sm overflow-hidden sticky top-0 z-30">
            ...
            @foreach($tabs as $tabId => $tab)
                <button onclick="switchTab('{{ $tabId }}')"
                    ...
                    class="tab-btn flex items-center gap-2.5 px-6 py-4 text-sm font-semibold whitespace-nowrap border-b-3 transition-all
                           {{ $loop->first ? 'border-slate-900 text-ink bg-cream' : 'border-transparent text-muted hover:text-inkSoft hover:bg-cream/80' }}">
    ...
    <div class="bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-6 py-4 border-b border-line flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold text-ink">Analiză Costuri</h2>
            <span class="ml-auto text-xs text-muted">AI + Conversații + Voice + Numere Telefon</span>
        </div>
        <div class="p-6">
            @if($costAnalysis['error'])
                <div class="bg-coralsoft border border-coral/30 text-coralh px-4 py-3 rounded-lg text-sm">{{ $costAnalysis['error'] }}</div>
            @else
                {{-- MoM comparison cards --}}
                @php $mom = $costAnalysis['mom'] ?? []; @endphp
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    <div class="bg-cream rounded-xl p-5 border border-line">
                        <p class="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Cost Luna Aceasta</p>
                        <p class="text-3xl font-bold text-ink">{{ number_format(($mom['this_month']['total'] ?? 0) / 100, 2) }} <span class="text-base font-normal text-muted">lei</span></p>
                        <div class="mt-3 space-y-1 text-xs">
                            <div class="flex justify-between"><span class="text-muted">AI (API calls)</span><span class="font-medium text-inkSoft">{{ number_format(($mom['this_month']['ai'] ?? 0) / 100, 2) }} RON</span></div>
                            <div class="flex justify-between"><span class="text-muted">Conversații</span><span class="font-medium text-inkSoft">{{ number_format(($mom['this_month']['msg'] ?? 0) / 100, 2) }} RON</span></div>
                            <div class="flex justify-between"><span class="text-muted">Voice (apeluri)</span><span class="font-medium text-inkSoft">{{ number_format(($mom['this_month']['voice'] ?? 0) / 100, 2) }} RON</span></div>
                            <div class="flex justify-between"><span class="text-muted">Numere telefon</span><span class="font-medium text-inkSoft">{{ number_format(($mom['this_month']['phone'] ?? 0) / 100, 2) }} RON</span></div>
                        </div>
                    </div>
                    <div class="bg-cream rounded-xl p-5 border border-line">
                        <p class="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Cost Luna Trecută</p>
                        <p class="text-3xl font-bold text-ink">{{ number_format(($mom['last_month']['total'] ?? 0) / 100, 2) }} <span class="text-base font-normal text-muted">lei</span></p>
                        <div class="mt-3 space-y-1 text-xs">
                            <div class="flex justify-between"><span class="text-muted">AI (API calls)</span><span class="font-medium text-inkSoft">{{ number_format(($mom['last_month']['ai'] ?? 0) / 100, 2) }} RON</span></div>
                            <div class="flex justify-between"><span class="text-muted">Conversații</span><span class="font-medium text-inkSoft">{{ number_format(($mom['last_month']['msg'] ?? 0) / 100, 2) }} RON</span></div>
                            <div class="flex justify-between"><span class="text-muted">Voice (apeluri)</span><span class="font-medium text-inkSoft">{{ number_format(($mom['last_month']['voice'] ?? 0) / 100, 2) }} RON</span></div>
                            <div class="flex justify-between"><span class="text-muted">Numere telefon</span><span class="font-medium text-inkSoft">{{ number_format(($mom['last_month']['phone'] ?? 0) / 100, 2) }} RON</span></div>
                        </div>
                    </div>
                    <div class="bg-cream rounded-xl p-5 border border-line">
                        <p class="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Variație Lunară</p>
                        @if($mom['change_pct'] !== null)
                            <p class="text-3xl font-bold {{ $mom['change_pct'] > 0 ? 'text-coral' : 'text-green-600' }}">
                                {{ $mom['change_pct'] > 0 ? '+' : '' }}{{ number_format($mom['change_pct'], 1) }}%
                            </p>
                            <p class="text-xs text-muted mt-2">{{ $mom['change_pct'] > 0 ? 'Creștere față de luna trecută' : 'Scădere față de luna trecută' }}</p>
                        @else
                            <p class="text-3xl font-bold text-muted">N/A</p>
                            <p class="text-xs text-muted mt-2">Nu există date luna trecută</p>
                        @endif

                        {{-- Phone numbers summary --}}
                        @if(count($costAnalysis['phone_numbers'] ?? []) > 0)
                            <div class="mt-4 pt-3 border-t border-line">
                                <p class="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Numere Telefon Active</p>
                                @foreach($costAnalysis['phone_numbers'] as $pn)
                                    <div class="flex justify-between items-center text-xs mb-1">
                                        <span class="text-muted font-mono">{{ $pn['number'] }}</span>
                                        <span class="font-medium text-inkSoft">{{ number_format($pn['monthly_cost_cents'] / 100, 2) }} lei/lună</span>
                                    </div>
                                @endforeach
                                <div class="flex justify-between items-center text-xs mt-2 pt-1 border-t border-line font-semibold">
                                    <span class="text-muted">Total lunar numere</span>
                                    <span class="text-ink">{{ number_format(($costAnalysis['phone_monthly_total'] ?? 0) / 100, 2) }} RON</span>
                                </div>
                            </div>
                        @endif
                    </div>
                ...
                <div class="mb-6">
                    <h4 class="text-sm font-medium text-inkSoft mb-3">Cost zilnic — ultimele 30 zile</h4>
                    <div class="flex items-end gap-[2px] h-28 bg-cream rounded-lg p-2 border border-line">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div>
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Top 10 Tenanți — Cost luna aceasta</h4>
                        @if(count($costAnalysis['top_tenants'] ?? []) > 0)
                            <div class="overflow-x-auto rounded-lg border border-line">
                                <table class="w-full text-sm">
                                    <thead class="bg-cream">
                                        <tr>
                                            <th class="text-left px-3 py-2.5 text-xs font-semibold text-muted">Tenant</th>
                                            <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">AI</th>
                                            <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Conv.</th>
                                            <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Voice</th>
                                            <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Tel.</th>
                                    ...
                                    <tbody class="divide-y divide-line">
                                        @foreach($costAnalysis['top_tenants'] as $t)
                                            <tr class="hover:bg-cream">
                                                ...
                                                <td class="px-3 py-2 text-right text-muted text-xs">{{ number_format($t['ai_cost'] / 100, 2) }}</td>
                                                <td class="px-3 py-2 text-right text-muted text-xs">{{ number_format(($t['msg_cost'] ?? 0) / 100, 2) }}</td>
                                                <td class="px-3 py-2 text-right text-muted text-xs">{{ number_format($t['voice_cost'] / 100, 2) }}</td>
                                                <td class="px-3 py-2 text-right text-muted text-xs">{{ number_format(($t['phone_cost'] ?? 0) / 100, 2) }}</td>
                                                <td class="px-3 py-2 text-right font-bold text-ink">{{ number_format($t['total'] / 100, 2) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                        ...
                        @else
                            <p class="text-sm text-muted">Nu există date.</p>
                        @endif
                    </div>

                    {{-- Cost by AI model --}}
                    <div>
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Cost per Model AI — luna aceasta</h4>
                        @if(count($costAnalysis['cost_by_model'] ?? []) > 0)
                            <div class="overflow-x-auto rounded-lg border border-line">
                                <table class="w-full text-sm">
                                    <thead class="bg-cream">
                                        <tr>
                                            <th class="text-left px-3 py-2.5 text-xs font-semibold text-muted">Provider / Model</th>
                                            <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Cereri</th>
                                            <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Tokeni</th>
                                    ...
                                    <tbody class="divide-y divide-line">
                                        @foreach($costAnalysis['cost_by_model'] as $m)
                                            <tr class="hover:bg-cream">
                                                ...
                                                <td class="px-3 py-2 text-right text-muted">{{ number_format($m['requests']) }}</td>
                                                <td class="px-3 py-2 text-right text-muted text-xs">{{ number_format(($m['input_tokens'] ?? 0) + ($m['output_tokens'] ?? 0)) }}</td>
                                                <td class="px-3 py-2 text-right font-bold text-ink">{{ number_format(($m['total_cost'] ?? 0) / 100, 3) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
    ...
    <div class="bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-6 py-4 border-b border-line flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold text-ink">Sănătate Servicii Externe</h2>
        </div>
        <div class="p-6">
            @if($serviceHealth['error'])
                <div class="bg-coralsoft border border-coral/30 text-coralh px-4 py-3 rounded-lg text-sm">{{ $serviceHealth['error'] }}</div>
            @elseif(empty($serviceHealth['providers'] ?? []))
                <p class="text-sm text-muted">Nu există date de metrici API disponibile.</p>
            @else
                {{-- Uptime cards --}}
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-{{ min(count($serviceHealth['providers']), 4) }} gap-4 mb-6">
                    @foreach($serviceHealth['providers'] as $provider)
                        ...
                        @endphp
                        <div class="bg-cream rounded-xl p-4 border border-line">
                            <div class="flex items-center justify-between mb-2">
                                <h3 class="text-sm font-semibold text-inkSoft uppercase tracking-wider">{{ ucfirst($provider) }}</h3>
                                <span class="w-3 h-3 rounded-full bg-{{ $uptimeColor }}-500"></span>
                            </div>
                            <p class="text-2xl font-bold text-ink">{{ number_format($uptimePct, 1) }}%</p>
                            <p class="text-xs text-muted">Uptime 24h &bull; {{ number_format($uptime['total']) }} cereri</p>
                            <div class="mt-2 flex items-center gap-1 text-xs">
                                @if($trend['trend'] === 'up')
                                    <svg class="w-3.5 h-3.5 text-coral" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 15l7-7 7 7"/></svg>
                                    <span class="text-coral">{{ $trend['current_rate'] }}% erori</span>
                                ...
                                @else
                                    <span class="text-muted">{{ $trend['current_rate'] }}% erori (stabil)</span>
                                @endif
                            </div>
                        </div>
                    @endforeach
                ...
                @foreach($serviceHealth['providers'] as $provider)
                    ...
                    @if(count($timeline) > 0)
                        <div class="mb-4">
                            <h4 class="text-sm font-medium text-inkSoft mb-2">{{ ucfirst($provider) }} — Timeline 48h</h4>
    ...
    <div class="bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-6 py-4 border-b border-line flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold text-ink">Tendințe Utilizare</h2>
        </div>
        <div class="p-6">
            @if($usageTrends['error'])
                <div class="bg-coralsoft border border-coral/30 text-coralh px-4 py-3 rounded-lg text-sm">{{ $usageTrends['error'] }}</div>
            @else
                {{-- Quick stats --}}
                ...
                <div class="mb-6">
                    <h4 class="text-sm font-medium text-inkSoft mb-3">Conversații zilnice — ultimele 30 zile</h4>
                ...
                {{-- Messages mini chart --}}
                <div class="mb-6">
                    <h4 class="text-sm font-medium text-inkSoft mb-3">Mesaje zilnice — ultimele 30 zile</h4>
                    <div class="flex items-end gap-px h-20">
                        @foreach($usageTrends['daily_stats'] as $ds)
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {{-- DAU (7 days) --}}
                    <div>
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Tenanți Activi Zilnic (DAU) — 7 zile</h4>
                        @if(count($usageTrends['dau'] ?? []) > 0)
                            ...
                            <div class="space-y-2">
                                @foreach($usageTrends['dau'] as $d)
                                    <div class="flex items-center gap-3">
                                        <span class="text-xs text-muted w-20 shrink-0">{{ $d['day'] }}</span>
                                        ...
                                        <span class="text-xs font-semibold text-inkSoft w-8 text-right">{{ $d['active_tenants'] }}</span>
                                    </div>
                                @endforeach
                            </div>
                        @else
                    ...
                    {{-- Busiest hours --}}
                    <div>
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Activitate pe Ore — ultimele 7 zile</h4>
                        <div class="flex items-end gap-px h-20">
                        ...
                        <div class="flex justify-between mt-1 text-[9px] text-muted">
                ...
                <div class="mt-8 pt-6 border-t border-line">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            <h4 class="text-sm font-semibold text-inkSoft">Sampling Calitate Q&A</h4>
                            <p class="text-xs text-muted mt-0.5">Generează 10 perechi aleatoare Întrebare & Răspuns cu scor de calitate</p>
                        </div>
                    ...
                    <div id="qa-loading" class="hidden flex items-center justify-center py-8">
                        ...
                        <span class="ml-3 text-sm text-muted">Se generează sample-urile...</span>
                    </div>
                    ...
                    {{-- Error container --}}
                    <div id="qa-error" class="hidden bg-coralsoft border border-coral/30 text-coralh px-4 py-3 rounded-lg text-sm"></div>
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-6 py-4 border-b border-line flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold text-ink">Analiză Erori</h2>
        </div>
        <div class="p-6">
            @if($errorAnalysis['error'])
                <div class="bg-coralsoft border border-coral/30 text-coralh px-4 py-3 rounded-lg text-sm">{{ $errorAnalysis['error'] }}</div>
            @else
                {{-- Error rate trend chart --}}
                @if(count($errorAnalysis['error_rate_trend'] ?? []) > 0)
                    <div class="mb-6">
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Rată Erori pe Oră — ultimele 48h</h4>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {{-- Top errors --}}
                    <div>
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Top Erori — ultimele 7 zile</h4>
                        @if(count($errorAnalysis['top_errors'] ?? []) > 0)
                            <div class="overflow-hidden rounded-lg border border-line">
                                <table class="w-full text-sm">
                                    <thead class="bg-cream">
                                        <tr>
                                            <th class="text-left px-3 py-2 text-xs font-semibold text-muted">Tip Eroare</th>
                                            <th class="text-left px-3 py-2 text-xs font-semibold text-muted">Provider</th>
                                    ...
                                    <tbody class="divide-y divide-line">
                                        @foreach($errorAnalysis['top_errors'] as $err)
                                            <tr class="hover:bg-cream">
                                                ...
                                                <td class="px-3 py-2 text-xs text-muted">{{ $err['provider'] }}</td>
                                                <td class="px-3 py-2 text-right font-semibold text-ink">{{ number_format($err['cnt']) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                    ...
                    <div class="space-y-6">
                        {{-- Failed jobs --}}
                        <div>
                            <h4 class="text-sm font-medium text-inkSoft mb-3">Job-uri Eșuate — ultimele 7 zile</h4>
                            @if(count($errorAnalysis['failed_jobs'] ?? []) > 0)
                                <div class="space-y-2">
                                    ...
                                    @foreach($errorAnalysis['failed_jobs'] as $fj)
                                        <div class="flex items-center gap-3">
                                            <span class="text-xs font-mono text-muted w-36 shrink-0 truncate" title="{{ $fj['full_class'] }}">{{ $fj['job_class'] }}</span>
                                            <div class="flex-1 bg-cream rounded-full h-3 overflow-hidden">
                                                <div class="h-full bg-red-500 rounded-full" style="width: {{ ($fj['count'] / $maxFailed) * 100 }}%"></div>
                                            </div>
                                            <span class="text-xs font-semibold text-inkSoft w-10 text-right">{{ $fj['count'] }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            @else
                        ...
                        {{-- Top error bots --}}
                        <div>
                            <h4 class="text-sm font-medium text-inkSoft mb-3">Top 5 Boți cu Erori</h4>
                            @if(count($errorAnalysis['top_error_bots'] ?? []) > 0)
                                <div class="space-y-2">
                                    ...
                                    @foreach($errorAnalysis['top_error_bots'] as $eb)
                                        <div class="flex items-center gap-3">
                                            <span class="text-xs text-muted w-36 shrink-0 truncate">{{ $eb['bot_name'] }}</span>
                                            <div class="flex-1 bg-cream rounded-full h-3 overflow-hidden">
                                                <div class="h-full bg-orange-500 rounded-full" style="width: {{ ($eb['error_count'] / $maxBotErr) * 100 }}%"></div>
                                            </div>
                                            <span class="text-xs font-semibold text-inkSoft w-10 text-right">{{ $eb['error_count'] }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            @else
                                <p class="text-sm text-muted">Niciun bot cu erori.</p>
                            @endif
                        </div>
                    </div>
                </div>
    ...
    <div class="bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-6 py-4 border-b border-line flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold text-ink">Transferuri & Solicitări Callback</h2>
        </div>
        <div class="p-6">
            @if($handoffCallback['error'])
                <div class="bg-coralsoft border border-coral/30 text-coralh px-4 py-3 rounded-lg text-sm">{{ $handoffCallback['error'] }}</div>
            @else
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {{-- Handoff section --}}
                    <div>
                        <h4 class="text-sm font-semibold text-inkSoft mb-3 uppercase tracking-wider">Handoff-uri (30 zile)</h4>
                        ...
                        @if($handoffCallback['handoff']['avg_resolution_minutes'])
                            <p class="text-xs text-muted mb-4">Timp mediu rezolvare: <span class="font-semibold text-inkSoft">{{ number_format($handoffCallback['handoff']['avg_resolution_minutes'], 0) }} min</span></p>
                        @endif
                            ...
                            <div class="space-y-1 mb-4">
                                @foreach($handoffCallback['handoff']['by_status'] as $status => $cnt)
                                    ...
                                    <div class="flex items-center gap-2">
                                        <span class="text-[10px] text-muted w-16 text-right">{{ $status }}</span>
                                        ...
                                        <span class="text-[10px] font-semibold text-muted w-8">{{ $cnt }}</span>
                                    </div>
                                @endforeach
                            </div>
                        @endif
                        ...
                        {{-- Unresolved handoffs --}}
                        <details class="group">
                            <summary class="text-xs font-medium text-muted cursor-pointer hover:text-ink">
                            ...
                            @if(count($handoffCallback['unresolved_handoffs'] ?? []) > 0)
                                <div class="mt-2 overflow-hidden rounded-lg border border-line">
                                    <table class="w-full text-xs">
                                        <thead class="bg-cream">
                                            <tr>
                                                <th class="text-left px-2 py-1.5 font-semibold text-muted">Tenant</th>
                                                <th class="text-left px-2 py-1.5 font-semibold text-muted">Bot</th>
                                                <th class="text-left px-2 py-1.5 font-semibold text-muted">Motiv</th>
                                        ...
                                        <tbody class="divide-y divide-line">
                                            @foreach($handoffCallback['unresolved_handoffs'] as $uh)
                                                <tr class="hover:bg-cream">
                                                    ...
                                                    <td class="px-2 py-1.5 text-muted">{{ Str::limit($uh['bot_name'], 15) }}</td>
                                                    <td class="px-2 py-1.5 text-muted">{{ Str::limit($uh['trigger_reason'], 20) }}</td>
                                                    <td class="px-2 py-1.5 text-right {{ $uh['age_hours'] > 24 ? 'text-coral font-semibold' : 'text-muted' }}">{{ $uh['age_hours'] }}h</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                    ...
                    {{-- Callback section --}}
                    <div>
                        <h4 class="text-sm font-semibold text-inkSoft mb-3 uppercase tracking-wider">Callback-uri (30 zile)</h4>
                        ...
                        <div class="flex gap-4 text-xs text-muted mb-4">
                            @if($handoffCallback['callback']['avg_confirm_minutes'])
                                <span>Confirmare medie: <span class="font-semibold text-inkSoft">{{ number_format($handoffCallback['callback']['avg_confirm_minutes'], 0) }} min</span></span>
                            @endif
                            @if($handoffCallback['callback']['avg_complete_minutes'])
                                <span>Finalizare medie: <span class="font-semibold text-inkSoft">{{ number_format($handoffCallback['callback']['avg_complete_minutes'], 0) }} min</span></span>
                            @endif
                        </div>
                            ...
                            @if(count($handoffCallback['pending_callbacks'] ?? []) > 0)
                                <div class="mt-2 overflow-hidden rounded-lg border border-line">
                                    <table class="w-full text-xs">
                                        <thead class="bg-cream">
                                            <tr>
                                                <th class="text-left px-2 py-1.5 font-semibold text-muted">Nume</th>
                                                <th class="text-left px-2 py-1.5 font-semibold text-muted">Telefon</th>
                                                <th class="text-left px-2 py-1.5 font-semibold text-muted">Data pref.</th>
                                                <th class="text-left px-2 py-1.5 font-semibold text-muted">Interval</th>
                                        ...
                                        <tbody class="divide-y divide-line">
                                            @foreach($handoffCallback['pending_callbacks'] as $pc)
                                                <tr class="hover:bg-cream">
                                                    ...
                                                    <td class="px-2 py-1.5 text-muted font-mono">{{ $pc['phone'] ?? '—' }}</td>
                                                    <td class="px-2 py-1.5 text-muted">{{ $pc['preferred_date'] ?? '—' }}</td>
                                                    <td class="px-2 py-1.5 text-muted">{{ $pc['preferred_time_slot'] ?? '—' }}</td>
                                                    <td class="px-2 py-1.5 text-right text-muted">{{ $pc['created_at'] }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
    ...
    <div class="bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-6 py-4 border-b border-line flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold text-ink">Profitabilitate per Tenant</h2>
        </div>
        <div class="p-6">
            @if($profitability['error'])
                <div class="bg-coralsoft border border-coral/30 text-coralh px-4 py-3 rounded-lg text-sm">{{ $profitability['error'] }}</div>
            @else
                {{-- Platform totals --}}
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                    ...
                    <div class="bg-teal-50 rounded-xl p-4 border border-teal-100">
                        ...
                        <p class="text-xl font-bold {{ ($pf['margin'] ?? 0) >= 0 ? 'text-teal-900' : 'text-coralh' }} mt-1">{{ number_format(($pf['margin'] ?? 0) / 100, 2) }} <span class="text-sm font-normal">lei</span></p>
                    </div>
                    <div class="bg-cream rounded-xl p-4 border border-line">
                        <p class="text-xs font-semibold text-muted uppercase tracking-wider">Marjă %</p>
                        <p class="text-xl font-bold {{ ($pf['margin_pct'] ?? 0) >= 0 ? 'text-emerald-700' : 'text-coralh' }} mt-1">{{ number_format($pf['margin_pct'] ?? 0, 1) }}%</p>
                    </div>
                </div>
                ...
                @if(count($profitability['tenants'] ?? []) > 0)
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            <thead class="bg-cream">
                                <tr>
                                    <th class="text-left px-3 py-2.5 text-xs font-semibold text-muted">#</th>
                                    <th class="text-left px-3 py-2.5 text-xs font-semibold text-muted">Tenant</th>
                                    <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Venituri</th>
                                    <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">AI</th>
                                    <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Conv.</th>
                                    <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Voice</th>
                                    <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Tel.</th>
                                    <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Cost Total</th>
                                    <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Marjă</th>
                            ...
                            <tbody class="divide-y divide-line">
                                @foreach($profitability['tenants'] as $idx => $pt)
                                    <tr class="hover:bg-cream">
                                        ...
                                        <td class="px-3 py-2 font-medium text-ink">{{ Str::limit($pt['tenant_name'], 25) }}</td>
                                        <td class="px-3 py-2 text-right text-emerald-700 font-medium">{{ number_format($pt['revenue'] / 100, 2) }}</td>
                                        <td class="px-3 py-2 text-right text-muted text-xs">{{ number_format($pt['ai_cost'] / 100, 2) }}</td>
                                        <td class="px-3 py-2 text-right text-muted text-xs">{{ number_format(($pt['msg_cost'] ?? 0) / 100, 2) }}</td>
                                        <td class="px-3 py-2 text-right text-muted text-xs">{{ number_format($pt['voice_cost'] / 100, 2) }}</td>
                                        <td class="px-3 py-2 text-right text-muted text-xs">{{ number_format(($pt['phone_cost'] ?? 0) / 100, 2) }}</td>
                                        <td class="px-3 py-2 text-right text-coral font-medium">{{ number_format($pt['total_cost'] / 100, 2) }}</td>
                                        <td class="px-3 py-2 text-right font-semibold {{ $pt['margin'] >= 0 ? 'text-emerald-700' : 'text-coralh' }}">{{ number_format($pt['margin'] / 100, 2) }}</td>
                                        <td class="px-3 py-2 text-right">
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium {{ $pt['margin_pct'] >= 50 ? 'bg-emerald-100 text-emerald-800' : ($pt['margin_pct'] >= 0 ? 'bg-amber-100 text-amber-800' : 'bg-coralsoft text-coralh') }}">
                ...
                @else
                    <p class="text-sm text-muted">Nu există date de profitabilitate luna aceasta.</p>
                @endif
            @endif
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-6 py-4 border-b border-line flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold text-ink">Knowledge Pipeline</h2>
        </div>
        <div class="p-6">
            @if($knowledgePipeline['error'])
                <div class="bg-coralsoft border border-coral/30 text-coralh px-4 py-3 rounded-lg text-sm">{{ $knowledgePipeline['error'] }}</div>
            @else
                        ...
                        @if(count($knowledgePipeline['agent_runs']['by_status'] ?? []) > 0)
                            <div class="mt-2 space-y-0.5">
                                @foreach($knowledgePipeline['agent_runs']['by_status'] as $st => $cnt)
                                    @php $stColor = match($st) { 'completed' => 'text-green-600', 'failed' => 'text-coral', 'running' => 'text-blue-600', default => 'text-muted' }; @endphp
                        ...
                        @if(count($knowledgePipeline['website_scans']['by_status'] ?? []) > 0)
                            <div class="mt-2 space-y-0.5">
                                @foreach($knowledgePipeline['website_scans']['by_status'] as $st => $cnt)
                                    @php $stColor = match($st) { 'completed' => 'text-green-600', 'failed' => 'text-coral', 'scanning', 'pending' => 'text-blue-600', default => 'text-muted' }; @endphp
                                    <p class="text-[10px] {{ $stColor }}">{{ $st }}: {{ $cnt }}</p>
                                @endforeach
                            </div>
                        @endif
                        ...
                        @if(count($knowledgePipeline['connectors']['by_type'] ?? []) > 0)
                            <div class="mt-2 space-y-0.5">
                                @foreach($knowledgePipeline['connectors']['by_type'] as $type => $cnt)
                                    <p class="text-[10px] text-muted">{{ $type }}: {{ $cnt }}</p>
                                @endforeach
                            </div>
                        @endif
                    </div>
                        ...
                        @if(count($knowledgePipeline['knowledge_items']['by_status'] ?? []) > 0)
                            <div class="mt-2 space-y-0.5">
                                @foreach($knowledgePipeline['knowledge_items']['by_status'] as $st => $cnt)
                                    @php $stColor = match($st) { 'ready' => 'text-green-600', 'failed' => 'text-coral', 'pending', 'processing' => 'text-blue-600', default => 'text-muted' }; @endphp
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {{-- Agent runs by slug --}}
                    <div>
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Rulări per Agent</h4>
                        @if(count($knowledgePipeline['agent_runs']['by_slug'] ?? []) > 0)
                            ...
                            <div class="space-y-2">
                                @foreach($knowledgePipeline['agent_runs']['by_slug'] as $as)
                                    <div class="flex items-center gap-3">
                                        <span class="text-xs text-muted w-32 shrink-0 truncate font-mono">{{ $as['agent_slug'] }}</span>
                                        <div class="flex-1 bg-cream rounded-full h-3 overflow-hidden">
                                            <div class="h-full bg-sky-500 rounded-full" style="width: {{ ($as['cnt'] / $maxSlug) * 100 }}%"></div>
                                        </div>
                                        <span class="text-xs text-muted w-20 text-right">{{ $as['cnt'] }} ({{ number_format($as['avg_tokens'] ?? 0) }}t)</span>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <p class="text-sm text-muted">Nu există rulări.</p>
                        @endif
                    </div>

                    {{-- Recent KB failures --}}
                    <div>
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Eșecuri Recente Knowledge Base</h4>
                        @if(count($knowledgePipeline['knowledge_items']['recent_failures'] ?? []) > 0)
                            <div class="overflow-hidden rounded-lg border border-line">
                                <table class="w-full text-xs">
                                    <thead class="bg-cream">
                                        <tr>
                                            <th class="text-left px-2 py-1.5 font-semibold text-muted">Titlu</th>
                                            <th class="text-left px-2 py-1.5 font-semibold text-muted">Bot ID</th>
                                    ...
                                    <tbody class="divide-y divide-line">
                                        @foreach($knowledgePipeline['knowledge_items']['recent_failures'] as $rf)
                                            <tr class="hover:bg-cream">
                                                ...
                                                <td class="px-2 py-1.5 text-muted">#{{ $rf['bot_id'] }}</td>
                                                <td class="px-2 py-1.5 text-coral font-mono">{{ Str::limit($rf['error_message'] ?? '—', 30) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
    ...
    <div class="bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-6 py-4 border-b border-line flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold text-ink">Webhook & Integrări</h2>
        </div>
        <div class="p-6">
            @if($integrationHealth['error'])
                <div class="bg-coralsoft border border-coral/30 text-coralh px-4 py-3 rounded-lg text-sm">{{ $integrationHealth['error'] }}</div>
            @else
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {{-- Connector health by type --}}
                    <div>
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Stare Conectori per Tip</h4>
                        @if(count($integrationHealth['connector_health'] ?? []) > 0)
                            <div class="space-y-3">
                                @foreach($integrationHealth['connector_health'] as $type => $statuses)
                                    ...
                                    @endphp
                                    <div class="bg-cream rounded-lg p-3 border border-line">
                                        <div class="flex items-center justify-between mb-2">
                                            <span class="text-sm font-medium text-inkSoft">{{ ucfirst($type) }}</span>
                                            <span class="text-xs px-2 py-0.5 rounded font-medium {{ $healthPct >= 80 ? 'bg-green-100 text-green-800' : ($healthPct >= 50 ? 'bg-amber-100 text-amber-800' : 'bg-coralsoft text-coralh') }}">{{ $healthPct }}% conectate</span>
                                        </div>
                        ...
                        @else
                            <p class="text-sm text-muted">Nu există conectori.</p>
                        @endif
                    </div>
                    ...
                    <div class="space-y-6">
                        {{-- Stale connectors --}}
                        <div>
                            <h4 class="text-sm font-medium text-inkSoft mb-3">Conectori Vechi (nesincronizați &gt;24h)</h4>
                            @if(count($integrationHealth['stale_connectors'] ?? []) > 0)
                                <div class="overflow-hidden rounded-lg border border-line">
                                    <table class="w-full text-xs">
                                        <thead class="bg-cream">
                                            <tr>
                                                <th class="text-left px-2 py-1.5 font-semibold text-muted">Tip</th>
                                                <th class="text-left px-2 py-1.5 font-semibold text-muted">Bot</th>
                                                <th class="text-left px-2 py-1.5 font-semibold text-muted">Status</th>
                                        ...
                                        <tbody class="divide-y divide-line">
                                            @foreach($integrationHealth['stale_connectors'] as $sc)
                                                <tr class="hover:bg-cream">
                                                    ...
                                                    <td class="px-2 py-1.5 text-muted">{{ Str::limit($sc['bot_name'], 20) }}</td>
                                                    <td class="px-2 py-1.5">
                                                        @php $scColor = match($sc['status']) { 'connected' => 'bg-green-100 text-green-700', 'error' => 'bg-coralsoft text-coralh', default => 'bg-cream text-muted' }; @endphp
                                                    ...
                                                    <td class="px-2 py-1.5 text-muted">
                                                        ...
                                                        @if($sc['hours_since_sync'])
                                                            <span class="text-coral">({{ $sc['hours_since_sync'] }}h)</span>
                                                        @endif
                                                    </td>
                                                </tr>
                                            @endforeach
                        ...
                        {{-- Chat events --}}
                        <div>
                            <h4 class="text-sm font-medium text-inkSoft mb-3">Evenimente Chat — ultimele 24h
                                <span class="ml-2 text-xs font-normal text-muted">(total: {{ number_format($integrationHealth['chat_event_total'] ?? 0) }})</span>
                            </h4>
                            @if(count($integrationHealth['chat_event_stats'] ?? []) > 0)
                                ...
                                <div class="space-y-1.5">
                                    @foreach($integrationHealth['chat_event_stats'] as $evt)
                                        <div class="flex items-center gap-2">
                                            <span class="text-[10px] text-muted w-32 shrink-0 truncate font-mono" title="{{ $evt['event_name'] }}">{{ $evt['event_name'] }}</span>
                                            <div class="flex-1 bg-cream rounded-full h-2.5 overflow-hidden">
                                                <div class="h-full bg-pink-500 rounded-full" style="width: {{ ($evt['cnt'] / $maxEvt) * 100 }}%"></div>
                                            </div>
                                            <span class="text-[10px] font-semibold text-muted w-10 text-right">{{ number_format($evt['cnt']) }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            @else
                                <p class="text-sm text-muted">Nu există evenimente în ultimele 24h.</p>
                            @endif
                        </div>
                    </div>
                </div>
    ...
    <div class="bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-6 py-4 border-b border-line flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold text-ink">A/B Testing</h2>
        </div>
        <div class="p-6">
            ...
            @else
                ...
                <div class="overflow-x-auto rounded-lg border border-line mb-6">
                    <table class="w-full text-sm">
                        <thead class="bg-cream">
                            <tr>
                                <th class="text-left px-3 py-2.5 text-xs font-semibold text-muted">Experiment</th>
                                <th class="text-left px-3 py-2.5 text-xs font-semibold text-muted">Bot</th>
                                <th class="text-left px-3 py-2.5 text-xs font-semibold text-muted">Tip</th>
                                <th class="text-left px-3 py-2.5 text-xs font-semibold text-muted">Status</th>
                                <th class="text-right px-3 py-2.5 text-xs font-semibold text-muted">Atribuiri</th>
                                <th class="text-left px-3 py-2.5 text-xs font-semibold text-muted">Metrică</th>
                                <th class="text-left px-3 py-2.5 text-xs font-semibold text-muted">Început</th>
                        ...
                        <tbody class="divide-y divide-line">
                            @foreach($abTesting['experiments'] as $exp)
                                ...
                                @endphp
                                <tr class="hover:bg-cream">
                                    <td class="px-3 py-2 font-medium text-ink">{{ Str::limit($exp['name'], 30) }}</td>
                                    <td class="px-3 py-2 text-muted text-xs">{{ Str::limit($exp['bot_name'], 20) }}</td>
                                    <td class="px-3 py-2 text-xs text-muted">{{ $exp['type'] ?? '—' }}</td>
                                    <td class="px-3 py-2">
                                        <span class="inline-flex px-2 py-0.5 rounded text-[10px] font-medium {{ $statusColors }}">{{ $exp['status'] ?? '—' }}</span>
                                    </td>
                                    <td class="px-3 py-2 text-right font-semibold text-ink">{{ number_format($exp['assignments_count'] ?? 0) }}</td>
                                    <td class="px-3 py-2 text-xs text-muted font-mono">{{ $exp['metric'] ?? '—' }}</td>
                                    <td class="px-3 py-2 text-xs text-muted">{{ $exp['started_at'] ?? '—' }}</td>
                                    <td class="px-3 py-2 text-xs text-muted">{{ $exp['ended_at'] ?? '—' }}</td>
                                </tr>
                                    ...
                                    <tr class="bg-cream/50">
                                        <td colspan="8" class="px-6 py-2">
                                            <div class="flex items-center gap-4 text-[10px]">
                                                <span class="text-muted font-medium">Distribuție variante:</span>
                ...
                @if(count($abTesting['prompt_version_stats'] ?? []) > 0)
                    <div>
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Versiuni Prompt per Bot</h4>
                        <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                            @foreach($abTesting['prompt_version_stats'] as $pv)
                                <div class="bg-cream rounded-lg p-3 border border-line text-center">
                                    <p class="text-[10px] text-muted">Bot #{{ $pv['bot_id'] }}</p>
                                    <p class="text-lg font-bold text-ink">{{ $pv['version_count'] }}</p>
                                    <p class="text-[10px] text-muted">{{ $pv['active_count'] }} active</p>
                                </div>
                            @endforeach
                        </div>
                    </div>
    ...
    <div class="bg-white rounded-xl border border-line overflow-hidden">
        <div class="px-6 py-4 border-b border-line flex items-center gap-3">
            ...
            <h2 class="text-lg font-semibold text-ink">Workers & Cozi</h2>
        </div>
        <div class="p-6">
            @if($workerStatus['error'] ?? null)
                <div class="bg-coralsoft border border-coral/30 text-coralh px-4 py-3 rounded-lg text-sm">{{ $workerStatus['error'] }}</div>
            @else
                {{-- Horizon status + quick stats --}}
                <div class="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-6">
                    {{-- Horizon status --}}
                    <div class="bg-cream rounded-xl p-4 border border-line">
                        <p class="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Horizon</p>
                        @php
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-lg font-bold text-ink">{{ $horizonLabel }}</span>
                        </div>
                        <p class="text-[10px] text-muted mt-1">{{ $workerStatus['masters_count'] ?? 0 }} master(s)</p>
                    </div>
                    ...
                    @foreach($workerStatus['queue_sizes'] ?? [] as $queueName => $size)
                        ...
                        <div class="{{ $qColor }} rounded-xl p-4 border">
                            <p class="text-xs font-semibold text-muted uppercase tracking-wider mb-2">Coadă: {{ $queueName }}</p>
                            <div class="flex items-center gap-2">
                                <span class="w-2 h-2 rounded-full {{ $qDot }}"></span>
                                <span class="text-lg font-bold {{ $qText }}">{{ number_format($size) }}</span>
                            </div>
                            <p class="text-[10px] text-muted mt-1">joburi în așteptare</p>
                        </div>
                    @endforeach
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {{-- Supervisors --}}
                    <div>
                        <h4 class="text-sm font-medium text-inkSoft mb-3">Supervisori</h4>
                                ...
                                @foreach($workerStatus['supervisors'] as $sv)
                                    ...
                                    <div class="rounded-lg p-3 border {{ $svColor }}">
                                        <div class="flex items-center gap-2 mb-1">
                                            ...
                                            <span class="text-xs font-semibold text-ink truncate">{{ Str::limit($sv['name'], 40) }}</span>
                                        </div>
                                        <div class="flex gap-4 text-[10px] text-muted">
                                            <span>Procese: <span class="font-semibold text-inkSoft">{{ $sv['processes'] }}</span></span>
                                            <span>Coadă: <span class="font-semibold text-inkSoft">{{ $sv['queue'] }}</span></span>
                                            <span>Balans: <span class="font-semibold text-inkSoft">{{ $sv['balance'] }}</span></span>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <p class="text-sm text-muted">Niciun supervisor activ.</p>
                        @endif
                        ...
                        @if(!empty($workerStatus['horizon_config'] ?? []))
                            <details class="mt-4 group">
                                ...
                                <div class="mt-2 bg-cream rounded-lg p-3 border border-line text-[10px] font-mono text-muted overflow-x-auto">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            <h4 class="text-sm font-medium text-inkSoft mb-3">
                                Throughput Joburi (ultimele 6h)
                                <span class="ml-2 text-xs font-normal text-muted">Procesate azi: {{ number_format($workerStatus['jobs_processed_today'] ?? 0) }}</span>
                            </h4>
                            @if(count($workerStatus['throughput'] ?? []) > 0)
                                <div class="flex items-end gap-2 h-20 bg-cream rounded-lg p-2 border border-line">
                            ...
                            @else
                                <p class="text-sm text-muted">Nu există date de throughput.</p>
                            @endif
                        </div>

                        {{-- Workload table --}}
                        <div>
                            <h4 class="text-sm font-medium text-inkSoft mb-3">Workload Horizon</h4>
                            @if(count($workerStatus['workload'] ?? []) > 0)
                                <div class="overflow-hidden rounded-lg border border-line">
                                    <table class="w-full text-xs">
                                        <thead class="bg-cream">
                                            <tr>
                                                <th class="text-left px-3 py-2 font-semibold text-muted">Coadă</th>
                                                <th class="text-right px-3 py-2 font-semibold text-muted">Lungime</th>
                                                <th class="text-right px-3 py-2 font-semibold text-muted">Așteptare (s)</th>
                                        ...
                                        <tbody class="divide-y divide-line">
                                            @foreach($workerStatus['workload'] as $wl)
                                                <tr class="hover:bg-cream">
                                                    ...
                                                    <td class="px-3 py-2 text-right {{ ($wl['length'] ?? 0) > 10 ? 'text-coral font-semibold' : 'text-muted' }}">{{ $wl['length'] }}</td>
                                                    <td class="px-3 py-2 text-right text-muted">{{ $wl['wait'] }}</td>
                                                    <td class="px-3 py-2 text-right text-muted">{{ $wl['processes'] }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                            ...
                            @else
                                <p class="text-sm text-muted">Nu există date de workload.</p>
                            @endif
                        </div>
                    </div>
                </div>
    ...
    {{-- Footer --}}
    <div class="text-center text-xs text-muted pb-4">
...
function switchTab(tabId) {
    ...
    document.querySelectorAll('.tab-btn').forEach(el => {
        el.classList.remove('border-slate-900', 'text-ink', 'bg-cream');
        el.classList.add('border-transparent', 'text-muted');
    });
    document.getElementById('tab-' + tabId).classList.remove('hidden');
    const btn = document.getElementById('tab-btn-' + tabId);
    btn.classList.add('border-slate-900', 'text-ink', 'bg-cream');
...
function fetchQASamples() {
    ...
    .then(data => {
        ...
        data.pairs.forEach(function(pair, idx) {
            const scoreColor = pair.score > 70 ? 'bg-green-100 text-green-800' : (pair.score >= 40 ? 'bg-amber-100 text-amber-800' : 'bg-coralsoft text-coralh');
            ...
            const breakdownHtml = [
                '<span class="text-muted">Lungime:</span> ' + (bd.length || 0) + '/30',
                '<span class="text-muted">Produse:</span> ' + (bd.products || 0) + '/20',
                '<span class="text-muted">Timp:</span> ' + (bd.response_time || 0) + '/20',
                '<span class="text-muted">Intenții:</span> ' + (bd.intents || 0) + '/15',
                '<span class="text-muted">Knowledge:</span> ' + (bd.knowledge || 0) + '/15',
            ].join(' &bull; ');

            const card = document.createElement('div');
            card.className = 'bg-cream rounded-lg border border-line p-4';
            card.innerHTML = `
                <div class="flex items-start justify-between gap-3 mb-2">
                    <div class="flex items-center gap-2">
                        <span class="text-xs font-bold text-muted">#${idx + 1}</span>
                        <span class="text-xs text-muted">${pair.bot_name}</span>
                        <span class="text-[10px] text-muted">${pair.created_at}</span>
                    </div>
                ...
                <div class="mb-2">
                    <p class="text-[10px] font-semibold text-muted uppercase tracking-wider mb-0.5">Întrebare</p>
                    <p class="text-sm text-ink bg-white rounded px-3 py-2 border border-line">${escapeHtml(question)}</p>
                </div>
                <div class="mb-2">
                    <p class="text-[10px] font-semibold text-muted uppercase tracking-wider mb-0.5">Răspuns</p>
                    <p class="text-sm text-inkSoft bg-white rounded px-3 py-2 border border-line">${escapeHtml(answer)}</p>
                </div>
                <div class="flex items-center gap-2 text-[10px]">
                    ${breakdownHtml}
                    ${pair.response_time_sec !== null ? '<span class="ml-auto text-muted">' + pair.response_time_sec + 's răspuns</span>' : ''}
                </div>
            `;

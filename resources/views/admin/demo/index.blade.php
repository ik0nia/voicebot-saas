...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-900">Demo pe landing pages</h1>
...
<form method="GET" class="mb-4 bg-white rounded-xl border border-slate-200 p-4 flex flex-wrap items-end gap-3">
    <div>
        <label class="block text-xs font-medium text-slate-600 mb-1">Perioadă (zile)</label>
        <select name="days" onchange="this.form.submit()" class="rounded-lg border border-slate-300 px-3 py-2 text-sm">
...
<div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <div class="text-xs text-slate-500 font-semibold uppercase tracking-wide">Vizualizate</div>
        <div class="text-3xl font-bold text-slate-900 mt-1">{{ number_format($stats['viewed']) }}</div>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <div class="text-xs text-slate-500 font-semibold uppercase tracking-wide">Mesaje</div>
        <div class="text-3xl font-bold text-slate-900 mt-1">{{ number_format($stats['messages']) }}</div>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <div class="text-xs text-slate-500 font-semibold uppercase tracking-wide">Calificate</div>
        <div class="text-3xl font-bold text-emerald-600 mt-1">{{ number_format($stats['qualified']) }}</div>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <div class="text-xs text-slate-500 font-semibold uppercase tracking-wide">Rata calificare</div>
        <div class="text-3xl font-bold text-slate-900 mt-1">{{ $stats['rate'] }}%</div>
    </div>
    <div class="bg-white rounded-xl border border-slate-200 p-4">
        <div class="text-xs text-slate-500 font-semibold uppercase tracking-wide">Înregistrări</div>
...
<div class="bg-white rounded-xl border border-slate-200 overflow-hidden">
    <table class="w-full text-sm">
        <thead>
            <tr class="text-left text-xs font-semibold text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
        ...
        <tbody>
            @forelse($stats['by_niche'] as $row)
                <tr class="border-b border-slate-100 hover:bg-slate-50">
                    ...
                    <td class="px-4 py-3 text-right tabular-nums text-slate-500">{{ number_format($row['messages']) }}</td>

(function() {
    ...
    if (!channelId) {
        console.warn('[Sambla Agent AI] Missing data-channel-id attribute.');
        return;
    }
    ...
    // Verifică domeniul
    fetch(apiBase + '/api/v1/agent AI/check-domain?channel_id=' + encodeURIComponent(channelId), {
        method: 'GET',
    ...
    .then(function(data) {
        if (!data.allowed) {
            console.warn('[Sambla Agent AI] Domain not authorized for this agent AI.');
            return;
        }
        ...
        style.textContent = '' +
            ...
            '#sambla-agent AI-iframe { width: 100%; height: calc(100% - 56px); border: none; }' +
            '@media (max-width: 480px) { #sambla-agent AI-frame-container { width: calc(100vw - 16px); right: -8px; height: 75vh; bottom: 72px; border-radius: 16px; } }';
        document.head.appendChild(style);

        // Creează widget-ul
        var widget = document.createElement('div');
        widget.id = 'sambla-agent AI-widget';

        // Container iframe
        var frameContainer = document.createElement('div');
        frameContainer.id = 'sambla-agent AI-frame-container';
        ...
        closeBtn.id = 'sambla-agent AI-close';
        closeBtn.innerHTML = '&#10005;';
        closeBtn.setAttribute('aria-label', 'Închide agent AI');
        header.appendChild(closeBtn);

        frameContainer.appendChild(header);

        // Iframe (lazy load) — uses the agent AI embed view
        var chatUrl = apiBase + '/api/v1/agent AI/' + encodeURIComponent(channelId) + '/frame';

        var iframe = document.createElement('iframe');
        iframe.id = 'sambla-agent AI-iframe';
        ...
        toggleBtn.addEventListener('click', function() {
            ...
            if (isOpen) {
                ...
                toggleBtn.setAttribute('aria-label', 'Închide agent AI');
    ...
    .catch(function(err) {
        console.warn('[Sambla Agent AI] Failed to load:', err);
    });
})();

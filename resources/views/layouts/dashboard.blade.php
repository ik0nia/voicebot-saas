            ...
            <header class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 lg:px-6 shrink-0">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div x-data="viewAsWidget()" class="relative">
                        ...
                        <div x-show="open" x-cloak @click.outside="open=false" class="absolute right-0 mt-2 w-[30rem] bg-white rounded-lg shadow-2xl ring-1 ring-slate-200 z-50 overflow-hidden flex flex-col" style="max-height: min(85vh, 44rem);">
                            <div class="p-3 border-b border-slate-100 shrink-0">
                                <input x-ref="search" type="text" x-model="q" @input.debounce.200ms="search()" placeholder="Caută tenant după nume sau slug…" class="w-full px-3 py-2 text-sm rounded border border-slate-200 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                            </div>
                            <div class="py-1 text-sm shrink-0">
                                <button type="button" @click="postStop()" class="w-full text-left px-4 py-2.5 hover:bg-slate-50 flex items-center justify-between">
                                    <span class="font-medium text-slate-700">Doar eu</span>
                                    @if(!$asTenantId && !$viewAll)<span class="text-slate-400 text-lg leading-none">●</span>@endif
                                </button>
                                <button type="button" @click="postAll()" class="w-full text-left px-4 py-2.5 hover:bg-slate-50 flex items-center justify-between">
                            ...
                            <div class="border-t border-slate-100 flex-1 overflow-y-auto py-1 text-sm min-h-[12rem]">
                                ...
                                <template x-for="t in results" :key="t.id">
                                    <button type="button" @click="postAs(t.id)" class="w-full text-left px-4 py-2.5 hover:bg-slate-50 flex items-center justify-between gap-3">
                                        <span class="min-w-0 flex-1 truncate">
                                            <span class="font-medium text-slate-800" x-text="t.name"></span>
                                            <span class="text-slate-400 text-xs" x-text="' · ' + (t.slug || '#' + t.id) + ' · ' + (t.plan || 'starter')"></span>
                                        </span>
                                        @if($asTenantId)
                                            <span class="text-orange-600 text-lg leading-none shrink-0" x-show="t.id === {{ (int) $asTenantId }}">●</span>
                                        @endif
                                    </button>
                                </template>
                            </div>
